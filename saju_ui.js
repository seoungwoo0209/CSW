...
function renderResourcePanel(resourceResult) {
  ...
  const axesHtml = axes.map(axis => {
    ...
    const barW = Math.round((axis.score / 220) * 100);
    return `
      <div style="
      ...
      ">
        ...
        <div style="display:flex;justify-content:space-between;margin-top:4px;">
          ...
          <span style="font-size:10px;color:#475569;">220</span>
        </div>
      </div>
    `;
  }).join("");
...
function renderDaeunScores(baseState, profileName = "overall") {
  ...
  decades.forEach((decade, idx) => {
    ...
    const daeunScore = window.SajuEngine.computeDaeunScore(daeunState, baseStrengthScore);
    const { overall, performance, friction, support } = daeunScore;
    const g = _grade(overall);

    console.log(`${decade.stem}${decade.branch}: 종합${overall} 성과${performance} 기반${support} 마찰${friction} (${g})`);
    ...
    if (info) {
      ...
      bEl.innerHTML = `
        <div class="breakdown-mini">
          <span title="현실 성과·실행력">성과:${performance}</span>
          <span title="버티는 힘·기반 안정">기반:${support}</span>
          <span title="마찰·소모 (낮을수록 편안)">마찰:${friction}</span>
        </div>
