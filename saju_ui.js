...
function renderIntuitionPanel(result) {
  ...
  const { categories, subs, overall, overloadRisk, typeName, typeDesc } = result;

  // ===== [FIX 4] 타입 헤더 카드 (점수 + 백분위 + 타입명) =====
  const typeHtml = `
    <div class="type-header-card">
      <div class="type-badge">${typeName || "균형통합형"}</div>
  ...
  function catCard(label, cat) {
    return `
      <div class="category-card">
        ...
        <div class="category-score grade-${cat.grade}">${cat.score}</div>
        <div class="category-percentile">${cat.percentileLabel || ''}</div>
        <div class="category-meta">${cat.grade}등급</div>
      </div>
    `;
  }
  ...
  const categoriesHtml = `
    <div class="intuition-categories">
      ${catCard("📊 통찰력", categories.Insight)}
      ${catCard("⏰ 기회 타이밍", categories.Timing)}
      ${catCard("💫 감응력", categories.Sensitivity)}
      ${catCard("🔮 예지력", categories.Premonition)}
    </div>
  `;
...
/**
 * 중분류 아이템 렌더링 (점수 + 백분위 + 스타일)
 */
function renderSubItem(sub) {
  const bonusText = sub.bonus !== 0 ? `<span class="sub-bonus">${sub.bonus > 0 ? '+' : ''}${sub.bonus}</span>` : '';
  const firedText = sub.fired && sub.fired.length > 0 ? `<div class="sub-fired">${sub.fired.join(', ')}</div>` : '';
  const styleText = sub.style ? `<span class="sub-style-badge">${sub.style}</span>` : '';
  const pctText   = sub.percentileLabel ? `<span class="sub-percentile">${sub.percentileLabel}</span>` : '';

  return `
    <div class="sub-item">
      <div class="sub-header">
        <div class="sub-name">${sub.id}. ${sub.name} ${styleText}</div>
        <div class="sub-score-wrap">
          <span class="sub-score grade-${sub.grade}">${sub.score}</span>
          ${bonusText}
          ${pctText}
        </div>
      </div>
      ${firedText ? `<div class="sub-meta">${firedText}</div>` : ''}
    </div>
  `;
}
