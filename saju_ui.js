...
function renderIntuitionPanel(result) {
  ...
  // ── 메인 총점 카드
  const mainHtml = `
    <div class="insight-main-card" style="
    ...
    ">
      ...
      ">${totalGrade}등급 · ${insightTotal >= 90 ? "최상위" : insightTotal >= 80 ? "상위" : insightTotal >= 70 ? "중상" : insightTotal >= 60 ? "중위" : "하위"}</div>
  ...
  const subsHtml = `
    ...
    <div style="display:flex; flex-direction:column; gap:8px;">
      ${(subs6 || []).map((sub, i) => {
        const pct = Math.round(sub.score);
        ...
        return `
          <div style="background:#f9fafb; border-radius:10px; padding:10px 14px; border:1px solid #e5e7eb;">
            ...
            <div style="background:#e5e7eb; border-radius:4px; height:5px; overflow:hidden;">
              <div style="background:${barColor}; height:100%; width:${pct}%; border-radius:4px; transition:width 0.6s;"></div>
            </div>
          </div>
        `;
      }).join('')}
