...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
[타고난 취업 기질]
${timeUnknown ? '' : `6하우스(일상 업무·고용): ${house6Str}\n10하우스(MC, 경력 정체성): ${house10Str}\n차트 지배행성(ASC ${ascSign}): ${ascRuler?.label || '?'} — ${ascRuler?.sign || '?'} ${ascRuler?.house || '?'}하우스\nMC 지배행성: ${mcRuler?.label || '?'} — ${mcRuler?.sign || '?'} ${mcRuler?.house || '?'}하우스\n`}수성(시험·면접·소통): ${mercury.sign}${timeUnknown ? '' : ` ${mercury.house}하우스`}
화성(경쟁력·추진력): ${mars.sign}${timeUnknown ? '' : ` ${mars.house}하우스`}
목성(기회 포착력): ${jupiterSign}
토성(인내·끈기): ${saturn.sign}${timeUnknown ? '' : ` ${saturn.house}하우스`}
...
${timeUnknown ? '' : `프로그레션 MC: ${progMcSign || '정보 없음'} (네이탈 MC ${mcSign}과 다르면 경력 테마 전환기)`}
...
각 마커는 단독 줄에 정확히 이 형태로 적어라. 마커 앞뒤로 다른 설명을 절대 덧붙이지 마라. 4개 섹션 전부 빠짐없이, 각자 요청된 분량을 줄이지 말고 작성해라 — 어떤 이유로도 마커를 생략하거나 한 섹션만 쓰고 끝내면 안 된다.
...
이 SECTION:suggestion 섹션 안에서만 한 문장으로 끝내라(마크다운 금지). 이 규칙은 이 섹션 안에만 적용되는 것이고, 위의 nature·timing·strength 섹션은 각각 요청한 분량과 형식을 그대로 지켜서 절대 줄이지 마라.)
...
function buildPromotionPrompt(body, sky) {
  ...
  const timeUnknownNote = timeUnknown ? '\n⚠️ 출생 시각 미상 — ASC·MC·하우스 배치는 신뢰할 수 없어 제외함. 행성 별자리 위치와 행성 간 각도 중심으로만 해석할 것.\n' : '';
  const prompt = `
너는 20년 경력의 서양 점성술 전문가야.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
[타고난 직장 내 위치]
${timeUnknown ? '' : `10하우스(MC, 경력 정체성): ${house10Str}\n`}토성(체계·권위에서 인정받는 방식): ${saturn.sign}${timeUnknown ? '' : ` ${saturn.house}하우스`}
태양(인정·가시성): ${sun.sign}${timeUnknown ? '' : ` ${sun.house}하우스`}
화성(협상력·추진력): ${mars.sign}${timeUnknown ? '' : ` ${mars.house}하우스`}
금성(처세·친화력): ${venus.sign}${timeUnknown ? '' : ` ${venus.house}하우스`}
${timeUnknown ? '' : `11하우스(인맥·동료 지지): ${house11Str}\n12하우스(숨은 정치·견제): ${house12Sign}\n2하우스(소득·연봉협상력): ${house2Str}\n`}목성(확장·인정받는 기회운): ${jupiterSign}
...
[섹션 구성 — 반드시 아래 4개 마커를 정확히 그대로 사용해서 구분할 것]
마커 앞뒤로 다른 설명을 절대 덧붙이지 마라. 4개 섹션 전부 빠짐없이, 각자 요청된 분량을 줄이지 말고 작성해라 — 어떤 이유로도 마커를 생략하거나 한 섹션만 쓰고 끝내면 안 된다.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
[타고난 이직 패턴]
천왕성(돌발적 변화·혁신 욕구): ${uranus.sign}${timeUnknown ? '' : ` ${uranus.house}하우스`}
${timeUnknown ? '' : `북노드(인생 방향·운명적 경로): ${northNodeSign} ${northNodeHouse}하우스\n9하우스(확장·도약형 이동): ${house9Sign}\n네이탈 MC: ${mcSign}\n`}목성(기회를 알아채고 잡는 타고난 감각): ${jupiterSign}

[지금 시점의 이직 타이밍 신호]
${transitWindowStr(uranusTransitWindow, '트랜짓 천왕성')} (MC·태양·10하우스 근접 시 급작스런 제안/변화 가능성 강함)
${timeUnknown ? '' : `${aspectStr(uranusLonNow, mcLon, '트랜짓 천왕성', '네이탈 MC')}\n${aspectStr(uranusLonNow, ascLon, '트랜짓 천왕성', '네이탈 ASC')}\n`}${timeUnknown ? '' : `프로그레션 MC: ${progMcSign || '정보 없음'}${mcChanged ? ' — 네이탈 MC와 달라짐 (경력 테마 전환기, 떠날 준비가 된 시기로 해석 가능)' : ' — 네이탈 MC와 동일 (아직 전환기 아님)'}`}
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
[타고난 사업가 기질]
${timeUnknown ? '' : `2하우스(자기자본): ${house2Str}\n8하우스(투자·타인자본·리스크): ${house8Str}\n`}화성(추진력·기업가 기질): ${mars.sign}${timeUnknown ? '' : ` ${mars.house}하우스`}
목성(과감함·확장운): ${jupiterSign}
태양(브랜드·리더십): ${sun.sign}${timeUnknown ? '' : ` ${sun.house}하우스`}
${timeUnknown ? '' : `5하우스(투기·창의적 사업): ${house5Sign}\nMC(나만의 사업 정체성): ${mcSign}, 지배행성 ${mcRuler?.label || '?'}(${mcRuler?.sign || '?'} ${mcRuler?.house || '?'}하우스)\n`}토성(장기 생존력·버텨내는 힘): ${saturn.sign}${timeUnknown ? '' : ` ${saturn.house}하우스`}
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
[타고난 경영 스타일]
${timeUnknown ? '' : `6하우스(조직·직원 관리): ${house6Str}\n7하우스(계약·파트너십): ${house7Str}\nMC(회사의 대외적 정체성·명성): ${mcStr}\n`}토성(책임을 다루는 방식): ${saturn.sign}${timeUnknown ? '' : ` ${saturn.house}하우스`}
화성(결단력·추진력): ${mars.sign}${timeUnknown ? '' : ` ${mars.house}하우스`}
수성(협상·커뮤니케이션 스타일): ${mercury.sign}${timeUnknown ? '' : ` ${mercury.house}하우스`}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!reply) {
      ...
      await logError(`gemini-career:${type}`, detail);
    ...
    await logError(`gemini-career:${req.body?.type || '?'}`, error?.message || error);
