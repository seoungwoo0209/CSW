...
function buildLovePrompt(body) {
  ...
  if (isSolo && Array.isArray(transits) && transits.length === 12 && Array.isArray(houses) && houses.length === 12) {
    ...
    const zrLoveBonus = timeUnknown ? 0 : (([5,7,8].includes(zrFortune?.l2House) || [5,7,8].includes(zrSpirit?.l2House)) ? 1 : 0);
    const profectionBonus = timeUnknown ? 0 : ([5,7].includes(profectionHouse) ? 1 : 0);
    const ctx = {
  ...
  if (!isSolo && Array.isArray(transits) && transits.length === 12 && Array.isArray(houses) && houses.length === 12) {
    ...
    const zrMarriageBonus = timeUnknown ? 0 : (([7].includes(zrFortune?.l2House) || [7].includes(zrSpirit?.l2House)) ? 1 : 0);
    const profectionBonus7H = timeUnknown ? 0 : (profectionHouse === 7 ? 1 : 0);
    const ctx2 = {
...
${timeUnknown
  ? `- 어떤 계기·환경에서 만나게 될 가능성이 높은지를 금성 별자리 기질과 화성(${mars.sign}) 에너지를 중심으로 구체적으로 짚어라(출생시가 불명확해 하우스 분석 제외)
- 화성(${mars.sign})이 보여주는 ${displayName}님의 접근 속도·에너지도 한 문장 섞어라(예: 화성이 활동적 별자리면 적극적으로 다가가는 편, 신중한 별자리면 천천히 확인하며 다가가는 편)
...
- 구체적으로 어떻게 행동하면 좋을지 실질적인 조언`;
  ...
  const progMoonStr = progMoonSign ? `프로그레션 달: ${progMoonSign}${timeUnknown ? '' : ` ${progMoonHouse}하우스`}` : '프로그레션 정보 없음';
  const progVenusStr = progVenusSign ? `프로그레션 금성: ${progVenusSign}${timeUnknown ? '' : ` ${progVenusHouse}하우스`} (지금 어떤 사랑에 끌리는지의 변화)` : '프로그레션 금성 정보 없음';
  ...
  const zrBlock = timeUnknown ? '' : [_zrLineStr(zrFortune, '포르투나'), _zrLineStr(zrSpirit, '스피릿')].filter(Boolean).join('\n');
  const timeUnknownNote = timeUnknown
    ? '\n⚠️ 출생 시각 미상 — 하우스 배치·ZR·프로펙션은 신뢰할 수 없어 제외함. 행성 별자리 위치와 행성 간 각도(aspect) 중심으로만 해석할 것.\n'
    : '';
...
[타고난 연애 기질 — 네이탈 차트]
금성(Venus): ${venus.sign}${timeUnknown ? '' : ` ${venus.house}하우스`} — 끌림의 방식·사랑을 표현하고 받는 방식
화성(Mars): ${mars.sign}${timeUnknown ? '' : ` ${mars.house}하우스`} — 욕망·연애에서의 추진력
달(Moon): ${moon.sign}${timeUnknown ? '' : ` ${moon.house}하우스`} — 정서적으로 원하는 것
${timeUnknown ? '' : `5하우스(연애·설렘): ${house5Str}\n7하우스(진지한 파트너십): ${house7Str}\n8하우스(깊은 정서적·성적 유대): ${house8Str}\n`}${timeUnknown ? '' : house5RulerStr}
${timeUnknown ? '' : ascRulerStr}
${jupVenusStr}
${timeUnknown ? '' : nodeStr}

[결혼·지속적 관계 — 토성·7하우스 지배행성]
토성(Saturn): ${saturn.sign}${timeUnknown ? '' : ` ${saturn.house}하우스`} — 책임감·관계를 얼마나 오래 지속시키는가
${timeUnknown ? '' : house7RulerStr}
${satVenusStr}
${satRulerStr}

${timeUnknown ? '' : `[ZR 타이밍 + 프로펙션 — 지금 어떤 주제가 열려 있는가]
${profectionStr}
${zrBlock || '(ZR 데이터 없음)'}
...
function buildReunionPrompt(body) {
  ...
  if (Array.isArray(transits) && transits.length === 12 && Array.isArray(houses) && houses.length === 12) {
    const venusRetroFlags = monthlyRetroFlags(transits, 'venus');
    if (venusRetroFlags) venusRetroFlags[nowMonthIdx] = venusRetro;
    const zrReunionBonus = timeUnknown ? 0 : (([7,8,12].includes(zrFortune?.l2House) || [7,8,12].includes(zrSpirit?.l2House)) ? 1 : 0);
    const profectionBonus = timeUnknown ? 0 : ([7,12].includes(profectionHouse) ? 1 : 0);
    ctx = {
  ...
  const progMoonStr = progMoonSign ? `프로그레션 달: ${progMoonSign}${timeUnknown ? '' : ` ${progMoonHouse}하우스`}` : '프로그레션 정보 없음';
  const progVenusStr = progVenusSign ? `프로그레션 금성: ${progVenusSign}${timeUnknown ? '' : ` ${progVenusHouse}하우스`}` : '프로그레션 금성 정보 없음';
  const eclipseStr = eclipseSignal
    ? (() => {
        const d = new Date(eclipseSignal.dateLocal);
        return `${d.getFullYear()}년 ${d.getMonth() + 1}월 ${d.getDate()}일 ${eclipseSignal.type}이 ${eclipseSignal.conjunctPoint}에 근접 — 관계의 중요한 전환점으로 해석 가능`;
  ...
  const timeUnknownNote = timeUnknown ? '\n⚠️ 출생 시각 미상 — ASC·하우스 배치·ZR·프로펙션은 신뢰할 수 없어 제외함. 행성 별자리 위치와 행성 간 각도 중심으로만 해석할 것.\n' : '';
  const prompt = `
너는 20년 경력의 서양 점성술 전문가야.
...
금성(Venus): ${venus.sign}${timeUnknown ? '' : ` ${venus.house}하우스`} — 과거 인연에 대한 애착 방식
화성(Mars): ${mars.sign}${timeUnknown ? '' : ` ${mars.house}하우스`}
달(Moon): ${moon.sign}${timeUnknown ? '' : ` ${moon.house}하우스`} — 미련·정서적 애착의 패턴
토성(Saturn): ${saturn.sign}${timeUnknown ? '' : ` ${saturn.house}하우스`} — 관계를 다시 시험하고 재정비하려는 성향
${timeUnknown ? '' : `7하우스(진지한 파트너십): ${house7Str}\n8하우스(깊은 정서적·성적 유대, 미련의 뿌리): ${house8Str}\n`}${timeUnknown ? '' : house7RulerStr}
${timeUnknown ? '' : ascRulerStr}
${satVenusStr}
${satRulerStr}
${jupVenusStr}
...
트랜짓 토성이 7/8하우스를 지나는 중인가: ${timeUnknown ? '(출생시 미상으로 확인 불가)' : saturnIn78 ? `예 (${transitSaturnHouse}하우스 — 관계의 재시험/재정비 시기)` : '아니오'}
${transitStr}
${progMoonStr}
${progVenusStr}
${eclipseStr}
...
[지금 시점의 재회 타이밍 신호]
금성 역행 여부: ${venusRetro ? '역행 중 (전통적으로 과거 인연이 다시 떠오르는 시기로 해석됨)' : '순행 중'}
${myLabel}의 트랜짓 토성이 7/8하우스를 지나는 중인가: ${timeUnknown ? '(출생시 미상으로 확인 불가)' : saturnIn78 ? `예 (${liveSaturnHouse}하우스 — 관계의 재시험/재정비 시기)` : '아니오'}
${eclipseStr}
