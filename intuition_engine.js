...
console.log("🔥 intuition_engine.js v7.0 로드");
...
(function () {
  ...
  // ── 기준 분포: mean/std — 5개 대표 명식 실측값 기반
  // 균형형이 z≈0, 구조독해형이 핵심3축에서 z≈2.5
  const DIST = {
    structure: { mean: 1.393, std: 0.472 },
    pattern:   { mean: 1.891, std: 0.677 },
    extract:   { mean: 0.710, std: 0.113 },
    risk:      { mean: 0.625, std: 0.249 },
    strategy:  { mean: 0.721, std: 0.253 },
    cohesion:  { mean: 1.519, std: 0.290 }
  };

  // ── z-score 기반 점수 변환 (GPT v7 명세)
  // 평균권 63~72, 상위 75~85, 최상위만 90+, 95+는 희귀
  function scoreFromZ(raw, dist) {
    const z = (raw - dist.mean) / Math.max(dist.std, 0.01);
    if (z <= -2.0) return 45;
    if (z <= -1.0) return 55 + 8  * (z + 1.0);
    if (z <=  0.0) return 63 + 9  * z;
    if (z <=  1.0) return 72 + 10 * z;
    if (z <=  2.0) return 82 + 6  * (z - 1.0);
    if (z <=  3.0) return 88 + 4  * (z - 2.0);
    return 92 + Math.min(5, 2 * (z - 3.0));
  }
  ...
  function compute(baseState) {
    ...
    // 로그
    console.log(`🔍 [v7.0] 십신기여: 인${ci_인성.toFixed(2)} 관${ci_관성.toFixed(2)} 식${ci_식상.toFixed(2)} 비${ci_비겁.toFixed(2)} 재${ci_재성.toFixed(2)}`);
    console.log(`🔍 [v7.0] 오행합산: 목${elWood.toFixed(2)} 화${elFire.toFixed(2)} 토${elEarth.toFixed(2)} 금${elMetal.toFixed(2)} 수${elWater.toFixed(2)}`);
    console.log(`🔍 [v7.0] 파생: 格순도=${geokPurity.toFixed(2)} 월령=${monthFit.toFixed(2)} 통근=${rootScore.toFixed(2)} 투간=${stemSupport.toFixed(2)} 잡음=${noiseIndex.toFixed(2)} 안정=${stabilityIndex.toFixed(2)} 삼각=${insightTriangle.toFixed(2)}`);

    // ══════════════════════════════════════════════════════
    // 6지표 계산 — GPT v7 명세: 배타적 재료 + z-score 환산
    // ══════════════════════════════════════════════════════

    // 1. 구조 독해력 (24%) — 목 + 관성 + geokPurity
    const raw_structure =
      1.9 * elWood
      + 1.4 * ci_관성
      + 1.1 * geokPurity
    ...
    if (elWood < 0.18) s_structure = Math.min(s_structure, 84);
    ...
    const raw_pattern =
      ...
      + 1.0 * rootScore
      + 0.9 * stemSupport
      + mildBonus(ci_식상, 0.14, 0.26, 0.15)
      - 0.4 * overPen(ci_재성, 0.30, 1.0)
      - 0.3 * noiseIndex;
    ...
    const raw_extract =
      ...
      + 0.9 * ci_식상
      + 0.3 * stabilityIndex
      - 0.5 * overPen(ci_인성, 0.34, 1.0)
      - 0.4 * overPen(elWater, 0.30, 1.0);

    const s_extract = scoreFromZ(raw_extract, DIST.extract);

    // 4. 리스크 식별력 (18%) — 관성 단독 + 금 보조
    const raw_risk =
      2.0 * ci_관성
      + 0.35 * elMetal
      - 0.2 * overPen(ci_재성, 0.31, 1.0);
    ...
    // 5. 전략 구상력 (18%) — 인성×관성 시너지 + 토
    const raw_strategy =
      2.8 * synergyIC
      + 0.9 * elEarth
      + 0.35 * Math.min(ci_비겁, 0.22)
      - 0.4 * overPenWeak(ci_비겁, 0.32, ci_인성, 0.22, 1.0);

    let s_strategy = scoreFromZ(raw_strategy, DIST.strategy);
    if (synergyIC < 0.04) s_strategy = Math.min(s_strategy, 82);
    ...
    // 2차: 핵심 3축 피크 보너스 (구조독해+리스크+전략)
    const tripleCore = (s_structure + s_risk + s_strategy) / 3;
    const peakBonus  = Math.max(0, tripleCore - 84) * 0.55;

    // 3차: 핵심 3축 2개 이상 80 미만 → 감점
    ...
    console.log(`📊 [v7.0] 6지표: 구조${sub1} 패턴${sub2} 추출${sub3} 리스크${sub4} 전략${sub5} 응집${sub6}`);
    console.log(`🎯 [v7.0] 통찰력 총점: ${insightTotal} | 90+조건: ${ninetyConditions}/6`);
  ...
  function _insightType(subs) {
    ...
    const total  = scores.reduce((a,b)=>a+b,0)/6;

    // 구조독해형: 구조독해 + 리스크 + 전략 3축이 강함
    if (s >= 90 && r >= 86 && st >= 84)
  ...
  console.log("✅ IntuitionEngine v7.0 로드 완료");
})();
