...
console.log("🔥 intuition_engine.js v3.1 로드");
...
(function() {
  ...
  // =========================================================
  // [v3.1] applyCategoryCapRules 제거 — 후처리 캡은 로직이 아님
  // 재료 설계(원재료 분리)로 이미 분리됨. 아래 dummy 유지로 호환성 보장
  ...
  function compute(baseState) {
    ...
    // [v3.1] 원국/대운 벡터 분리 — 능력 분석은 baseVectors 우선 사용
    // saju_engine.js v3.0이 baseVectors를 제공하면 사용, 없으면 vectors 폴백
    const baseV = baseState.vectors?.baseVectors || baseState.vectors;

    // 10십신 → 5축 브리지
    const rawTenGods = baseV.tenGods;
    ...
    console.log("🔍 [v3.1] baseVectors 5축 합계:", tgMag.toFixed(2), JSON.stringify(tg0));
    ...
    // =========================================================
    // 카테고리 집계 (후처리 캡 없음 — 재료 설계로 이미 분리됨)
    // =========================================================
