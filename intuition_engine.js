...
console.log("🔥 intuition_engine.js v8.0 로드");
...
(function () {
  ...
  // ── 기준 분포: mean/std — GPT v8, 6개 대표 명식 실측 역산
  // 핵심3축(structure/risk/strategy)에서 구조독해형 z≈2.5
  // 토안정형은 핵심3축에서 z<0으로 자동 분리됨
  const DIST = {
    structure: { mean: 1.501, std: 0.554 },
    pattern:   { mean: 1.888, std: 0.721 },
    extract:   { mean: 0.670, std: 0.129 },
    risk:      { mean: 0.637, std: 0.252 },
    strategy:  { mean: 0.512, std: 0.295 },
    cohesion:  { mean: 1.544, std: 0.380 }
  };
  ...
  function scoreFromZ(raw, dist) {
    ...
    if (z <= -2.0) return 45;
    if (z <= -1.0) return 50 + 8  * (z + 1.0);   // -2~-1: 50~58
    if (z <=  0.0) return 58 + 12 * (z + 1.0 - 1.0);  // -1~0: 58~70
  ...
  function compute(baseState) {
    ...
    // 로그
    console.log(`🔍 [v8.0] 십신기여: 인${ci_인성.toFixed(2)} 관${ci_관성.toFixed(2)} 식${ci_식상.toFixed(2)} 비${ci_비겁.toFixed(2)} 재${ci_재성.toFixed(2)}`);
    console.log(`🔍 [v8.0] 오행합산: 목${elWood.toFixed(2)} 화${elFire.toFixed(2)} 토${elEarth.toFixed(2)} 금${elMetal.toFixed(2)} 수${elWater.toFixed(2)}`);
    console.log(`🔍 [v8.0] 파생: 格순도=${geokPurity.toFixed(2)} 월령=${monthFit.toFixed(2)} 통근=${rootScore.toFixed(2)} 투간=${stemSupport.toFixed(2)} 잡음=${noiseIndex.toFixed(2)} 안정=${stabilityIndex.toFixed(2)} 삼각=${insightTriangle.toFixed(2)}`);
    ...
    else if (elWood < 0.18) s_structure = Math.min(s_structure, 84);
    // 95+ 캡
    if (!(ci_관성 >= 0.26 && elWood >= 0.24 && geokPurity >= 0.72))
      s_structure = Math.min(s_structure, 92);

    // 2. 패턴 해석력 (14%) — 인성 + rootScore + stemSupport (관성/토/목 반복 금지)
    const raw_pattern =
      2.05 * ci_인성
      + 1.05 * rootScore
      + 0.95 * stemSupport
    ...
    let s_pattern = scoreFromZ(raw_pattern, DIST.pattern);
    // 인성 root 부족 캡
    if (rootScore < 0.25) s_pattern = Math.min(s_pattern, 88);

    // 3. 핵심 추출력 (13%) — 금 + 식상 (토/관성 직접 금지)
    const raw_extract =
      1.95 * elMetal
      + 0.95 * ci_식상
      + 0.25 * stabilityIndex   // 아주 약하게
    ...
    else if (elMetal < 0.16) s_extract = Math.min(s_extract, 84);

    // 4. 리스크 식별력 (18%) — 관성 + 금 보조 (토/수 직접 금지, noise 직접 보너스 금지)
    const raw_risk =
      1.75 * ci_관성
      + 0.55 * elMetal
      + 0.20 * stabilityIndex   // 약하게
    ...
    // 금 약하면 고점 캡
    if (elMetal < 0.10)      s_risk = Math.min(s_risk, 86);
    else if (elMetal < 0.14) s_risk = Math.min(s_risk, 90);
    // noise 높으면 캡
    if (noiseIndex > 0.24) s_risk = Math.min(s_risk, 88);
    // 95+ 캡
    if (!(ci_관성 >= 0.29 && noiseIndex <= 0.18))
      s_risk = Math.min(s_risk, 92);

    // 5. 전략 구상력 (18%) — 인성×관성 시너지 + 토 보조 (토 단독 고가산 금지)
    const raw_strategy =
      3.10 * synergyIC           // 인성×관성 시너지 (핵심)
      + 0.38 * elEarth           // 토: 약한 보조만
      + 0.25 * Math.min(ci_비겁, 0.22)
      - 0.45 * overPenWeak(ci_비겁, 0.32, ci_인성, 0.22, 1.0)
      - 0.22 * noiseIndex;

    let s_strategy = scoreFromZ(raw_strategy, DIST.strategy);
    // 핵심 캡: 토 많아도 시너지 낮으면 고점 금지
    if      (synergyIC < 0.040) s_strategy = Math.min(s_strategy, 78);
    else if (synergyIC < 0.055) s_strategy = Math.min(s_strategy, 84);
    if (elEarth > 0.32 && synergyIC < 0.060)
      s_strategy = Math.min(s_strategy, 82);
    // 95+ 캡
    if (!(synergyIC >= 0.055 && elEarth >= 0.20 && ci_비겁 <= 0.30))
      s_strategy = Math.min(s_strategy, 92);

    // 6. 통찰 응집력 (13%) — stabilityIndex + monthFit (토 직접 가산 금지)
    const raw_cohesion =
      1.55 * stabilityIndex
      + 1.10 * monthFit
      + 0.45 * geokPurity        // 약보조
    ...
    let s_cohesion = scoreFromZ(raw_cohesion, DIST.cohesion);
    // 구조축 약하면 응집 고점 금지
    if (elWood < 0.16 && ci_관성 < 0.20) s_cohesion = Math.min(s_cohesion, 82);
    if (monthFit < 0.50)                  s_cohesion = Math.min(s_cohesion, 84);
    ...
    console.log(`📊 [v8.0] 6지표: 구조${sub1} 패턴${sub2} 추출${sub3} 리스크${sub4} 전략${sub5} 응집${sub6}`);
    console.log(`🎯 [v8.0] 통찰력 총점: ${insightTotal} | 90+조건: ${ninetyConditions}/6`);
  ...
  console.log("✅ IntuitionEngine v8.0 로드 완료");
})();
