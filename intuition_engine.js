...
(function () {
  ...
  function scoreFromZ(raw, dist) {
    ...
    // 목표 분포: 하위 68~72 / 보통 73~79 / 상위 80~87 / 최상위 88~92
    // 단조 증가 보장 — 순위 불변
    if (z <= -2.5) return 60;
    if (z <= -2.0) return 60 + 4   * (z + 2.5);   // 60→62
    if (z <= -1.0) return 62 + 6   * (z + 2.0);   // 62→68
    if (z <=  0.0) return 68 + 6   * (z + 1.0);   // 68→74
    if (z <=  1.0) return 74 + 8   * z;            // 74→82
    if (z <=  2.0) return 82 + 8   * (z - 1.0);   // 82→90
    if (z <=  2.7) return 90 + (3 / 0.7) * (z - 2.0); // 90→93
    if (z <=  3.2) return 93 + (3 / 0.5) * (z - 2.7); // 93→96
    return 96 + Math.min(1, z - 3.2);
  }
