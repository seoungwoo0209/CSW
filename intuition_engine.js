...
(function () {
  ...
  // ── raw → 0~100 (tanh 스케일, 진폭 38)
  function toScore(raw, scale) {
    return clamp(50 + 38 * Math.tanh(safeN(raw) / Math.max(0.15, scale)), 2, 98);
  }
  ...
  function compute(baseState) {
    ...
    // 1. 구조 독해력 (22%)
    // 목구조성·인성해석·관성긴장·格순도·월령·insightTriangle
    const raw_structure =
      1.70 * elWood           // 목: 구조 골격
      + 1.25 * ci_인성        // 인성: 해석
      + 1.20 * ci_관성        // 관성: 긴장·질서
      + 1.20 * geokPurity     // 格 순도
      + 0.80 * monthFit       // 월령 적합도
      + 1.10 * insightTriangle // 삼각 결합 보너스
      - 0.80 * noiseIndex
      - overPen(ci_비겁, 0.70, 0.40)  // 비겁 과다: 자기증폭
    ...
    // 2. 패턴 해석력 (16%)
    // 인성 최우선·목 보조·rootScore·stemSupport·insightTriangle
    const raw_pattern =
      1.80 * ci_인성
      + 1.15 * elWood
      ...
      + 0.80 * rootScore
      + 0.60 * stemSupport
      + mildBonus(ci_식상, 0.15, 0.55, 0.06)  // 적정 식상: 출력 보조
      - overPenWeak(ci_식상, 0.65, ci_인성, 0.40, 0.50) // 식상 과다+인성 약 → 분산
      - overPen(ci_재성, 0.65, 0.35)           // 재성 과다: 현실 편향
      - 0.35 * noiseIndex;

    const s_pattern = toScore(raw_pattern, 0.55);

    // 3. 핵심 추출력 (14%)
    // 금절단·인성·식상일부·관성일부·stabilityIndex
    const raw_extract =
      1.70 * elMetal
      + 1.15 * ci_인성
      + 0.55 * ci_식상         // 식상: 정리·결론화
      + 0.55 * ci_관성         // 관성: 현실 절단
      + 0.70 * stabilityIndex
      - overPen(ci_인성, 0.80, 0.45)  // 인성 과다: 생각만 많음
      - overPen(elWater, 1.20, 0.40)  // 수 과다: 흔들림
      - 0.70 * noiseIndex;

    const s_extract = toScore(raw_extract, 0.54);

    // 4. 리스크 식별력 (18%)
    // 관성 최우선·금·수보조(소량)·monthFit·stabilityIndex
    const raw_risk =
      1.85 * ci_관성
      + 1.15 * elMetal
      + 0.40 * elWater          // 수: 조짐 감지 보조(소량)
      + 0.50 * monthFit
      + 0.50 * stabilityIndex
      + IL.sensitivityBonus     // 형/충 적정: 미세 이상신호 보조
      - overPen(ci_재성, 0.65, 0.40)  // 재성 과다: 낙관편향
      - overPen(ci_식상, 0.65, 0.35)  // 식상 과다: 낙관·표현 편향
      - 0.25 * noiseIndex;      // 불안≠리스크: 소폭만

    const s_risk = toScore(raw_risk, 0.56);

    // 5. 전략 구상력 (16%)
    // 인성·관성·인성×관성 시너지·토·식상일부·비겁적정
    const raw_strategy =
      1.50 * ci_인성
      + 1.45 * ci_관성
      + 0.85 * elEarth
      ...
      + 0.40 * Math.min(ci_비겁, 0.55)  // 비겁 적정: 주체성 보조
      - overPen(ci_식상, 0.70, 0.35)
      - overPenWeak(ci_비겁, 0.75, ci_인성, 0.40, 0.45)  // 비겁 과다+인성 약 → 독선
      - 0.35 * noiseIndex;

    const s_strategy = toScore(raw_strategy, 0.58);

    // 6. 통찰 응집력 (14%)
    // 格순도·stabilityIndex·monthFit·인성×관성·insightTriangle
    const raw_cohesion =
      1.40 * geokPurity
      + 1.10 * stabilityIndex
      + 0.80 * monthFit
      ...
      - 1.00 * noiseIndex
      - calcDispersionPen(ci_인성, ci_관성, elWood, elWater);
