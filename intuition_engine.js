...
(function () {
  ...
  // ── 전체 점수 스케일 보정 — 순위 불변 단조 변환
  // 하위(45) 최대 +16, 중위(73) +6, 상위(88) +2, 최상위(95+) 거의 0
  function subScale(s) {
    const t = Math.max(0, Math.min(1, (s - 45) / 53));
    return Math.min(98, s + 16 * Math.pow(1 - t, 1.2));
  }
  ...
  function compute(baseState) {
    ...
    // 각 서브점수에 스케일 보정 적용 (n90 조건은 보정 전 원본값 기준)
    const sub1 = Math.round(subScale(s_struct));
    const sub2 = Math.round(subScale(s_risk));
    const sub3 = Math.round(subScale(s_essence));

    // 총점도 보정된 서브값 기준으로 재합산
