...
(function() {
  ...
  function normalizeAuto(obj) {
    const vals = Object.values(obj).map(v => +v || 0);
    const sum = vals.reduce((a, b) => a + b, 0) || 1;

    // 이미 비율에 가까운지 감지
    if (sum > 0.95 && sum < 1.05) return { ...obj };

    const out = {};
    for (const k in obj) out[k] = (+obj[k] || 0) / sum;
    return out;
  }
  ...
  function compute(baseState) {
    ...
    // ===== 종합 결과 4개 =====
    const综合1 = Math.round(0.45 * TimingAvg + 0.35 * InsightAvg + 0.20 * (100 - OverloadRisk));
    const 综合2 = Math.round(0.55 * sub11.score + 0.25 * sub12.score + 0.20 * sub13.score);
    const 综合3 = Math.round(0.55 * sub19.score + 0.25 * sub16.score + 0.20 * sub20.score);
    const 综合4 = Math.round(0.50 * sub3.score + 0.20 * sub7.score + 0.30 * (100 - OverloadRisk));
    ...
    return {
      ...
      // 종합 결과
      overall: {
        비즈니스촉: { score: 综合1, grade: grade(综合1), percent: percentBand(综合1) },
        연애호감촉: { score: 综合2, grade: grade(综合2), percent: percentBand(综合2) },
        예지몽: { score: 综合3, grade: grade(综合3), percent: percentBand(综合3) },
        위기회피: { score: 综合4, grade: grade(综合4), percent: percentBand(综合4) }
      },
