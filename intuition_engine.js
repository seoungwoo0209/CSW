/* =========================================================
   직관(직장 통찰력) 분석 엔진 (intuition_engine.js) v3.0
   ─────────────────────────────────────────────────────────
   ...
   3지표 (직장 통찰력):
     문제 포착력 (42%) / 위험 감지력 (34%) / 본질 해석력 (24%)
   ========================================================= */

console.log("🔥 intuition_engine.js v3.0 로드 시작");
...
(function () {
  ...
  function scoreFromZ(raw, dist) {
    ...
    if (z <=  2.0) return 82 + 8   * (z - 1.0);
    if (z <=  2.7) return 90 + (4 / 0.7) * (z - 2.0);
    if (z <=  3.2) return 94 + (2 / 0.5) * (z - 2.7);
  ...
  function compute(baseState) {
    ...
    if (elWood < 0.14)    s_struct = Math.min(s_struct, 78);
    else if (elWood < 0.18) s_struct = Math.min(s_struct, 84);
    if (ci_관성 < 0.18)   s_struct = Math.min(s_struct, 86);
    ...
    // ── 총점 합성
    const baseTotal = 0.42 * s_struct + 0.34 * s_risk + 0.24 * s_essence;
    const pairCore  = (s_struct + s_risk) / 2;
    const pairBonus = Math.max(0, pairCore - 84) * 0.32;
    const weakPenalty = [s_struct, s_risk].filter(v => v < 78).length >= 2 ? 2.0 : 0;

    let insightTotal = baseTotal + pairBonus - weakPenalty;
    insightTotal = clamp(insightTotal, 45, 97);

    // 90+ 허용: 3개 중 2개 이상
    const n90 = [s_struct >= 90, s_risk >= 88, s_essence >= 84].filter(Boolean).length;
    if (insightTotal >= 90 && n90 < 2) insightTotal = Math.min(insightTotal, 89);
    ...
    console.log(`📊 [IE v3] 문제포착${sub1} 위험감지${sub2} 본질해석${sub3} → 총점${insightTotal}`);
