/* =========================================================
   직관(직장 통찰력) 분석 엔진 (intuition_engine.js) v3.2
   ─────────────────────────────────────────────────────────
   ...
   v3.2 변경 (점수 분포 재설계):
     · scoreFromZ 전 구간 재매핑 — 목표 분포 달성
       하위 68~72 / 보통 73~79 / 상위 80~87 / 최상위 88~92
     · subScale 제거 — scoreFromZ가 분포 직접 처리
     · raw 계산식 / 계수 / 반환 구조 변경 없음
     · 단조 증가 보장 → 상대 순위 불변
...
console.log("🔥 intuition_engine.js v3.2 로드 시작");
...
(function () {
  ...
  function scoreFromZ(raw, dist) {
    ...
    // 목표 분포 기반 구간 + 일괄 +11
    // 상위권 구분 유지를 위해 85 이상은 압축 적용
    let s;
    if (z <= -2.5)      s = 60;
    else if (z <= -2.0) s = 60 + 4   * (z + 2.5);
    else if (z <= -1.0) s = 62 + 6   * (z + 2.0);
    else if (z <=  0.0) s = 68 + 6   * (z + 1.0);
    else if (z <=  1.0) s = 74 + 8   * z;
    else if (z <=  2.0) s = 82 + 8   * (z - 1.0);
    else if (z <=  2.7) s = 90 + (3 / 0.7) * (z - 2.0);
    else if (z <=  3.2) s = 93 + (3 / 0.5) * (z - 2.7);
    else                s = 96 + Math.min(1, z - 3.2);
    // +11 가산, 85 초과분은 절반만 가산 (상위 구분 유지)
    const boost = s <= 85 ? 11 : 11 - (s - 85) * 0.55;
    return Math.min(97, s + Math.max(0, boost));
  }
  ...
  function compute(baseState) {
    ...
    insightTotal = clamp(insightTotal, 60, 97);

    // 90+ 차단 완화: 기존 n90<2 → n90<1 (1개 이상이면 90+ 허용)
    const n90_pre = [s_struct >= 88, s_risk >= 86, s_essence >= 84].filter(Boolean).length;
    if (insightTotal >= 90 && n90_pre < 1) insightTotal = Math.min(insightTotal, 89);

    insightTotal = Math.round(clamp(insightTotal, 60, 97));

    // n90 조건은 scoreFromZ 출력 기준
    const n90 = [s_struct >= 88, s_risk >= 86, s_essence >= 84].filter(Boolean).length;
    if (insightTotal >= 92 && n90 < 1) insightTotal = Math.min(insightTotal, 91);

    const sub1 = Math.round(clamp(s_struct,  60, 97));
    const sub2 = Math.round(clamp(s_risk,    60, 97));
    const sub3 = Math.round(clamp(s_essence, 60, 97));

    // 총점: 보정된 서브값 기준 재합산
    insightTotal = Math.round(clamp(
      0.40 * sub1 + 0.22 * sub2 + 0.38 * sub3,
      60, 97
    ));
  ...
  function _fallback() {
    return {
      insightTotal:68, typeName:"균형형", typeDesc:"",
      comment: { strength:"(데이터 없음)", weakness:"" },
      subs6:[], categories:{ Insight:{ score:68, grade:"C", percent:"중위" } },
      overloadRisk:0,
    };
  }

  window.IntuitionEngine = { compute };
  console.log("✅ IntuitionEngine v3.2 로드 완료");
})();
