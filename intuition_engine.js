...
(function() {
  ...
  function compute(baseState) {
    ...
    const sub1 = subSkillStyled(
      1, "구조 파악력",
      [
        { name: "골격형",  raw: 1.7*el.wood + 1.2*tg.인성 + 0.9*tg.관성 + 0.7*geokPurity
    ...
    const sub2 = subSkillStyled(
      ...
      [
        ...
        // 실행/기회 편향 패널티 — 이런 명식은 패턴 해석보다 즉흥 반응
        { cond: tg.식상 >= 0.27 && tg.인성 < 0.15,               add: -7, name: "식상 과다+인성 부족" },
        { cond: tg.재성 >= 0.27 && tg.인성 < 0.15,               add: -6, name: "재성 과다+인성 부족" },
    ...
    const sub4 = subSkillStyled(
      ...
      [
        ...
        { name: "정리형",  raw: 1.4*tg.인성 + 1.1*el.metal + 0.6*tg.관성
                               - 0.30*noise
