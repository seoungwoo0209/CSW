...
(function() {
  ...
  function compute(baseState) {
    ...
    // ── 통찰력 전용 변수 ──
    // 格 순도: buildState.geok가 있으면 사용, 없으면 중립 0.5
    ...
    // 구조독해 삼각 지수: 인성+관성+목이 균형 있을 때만 폭발적 상승
    // min()이므로 어느 하나라도 약하면 값이 낮아짐
    const insightTriangle = Math.min(tg.인성, tg.관성, el.wood);

    // 타이밍 전용
    const holdBias = tg.비겁 * 0.5 + tg.인성 * 0.35; // 회수 지연 지수
    const cautious  = tg.관성 * 0.5;                  // 숙고 지연 지수

    // ===== Insight (1~5) — 구조독해형 통찰 =====
    // ★ 핵심 원칙:
    //   주재료: 인성·관성·목·금·格순도
    //   보조재료: 수(미세조짐), 충형파해(경계민감) — 소량만
    //   금지재료: spike 직접가산, noise 직접보너스, el.water 주재료, 감각/스파크 스타일
    //   alone 금지: 인성alone / 관성alone / 목alone으로는 고득점 불가
    //              → insightTriangle로 "2개 이상 결합" 강제
    // 1) 구조 파악력
    //    주재료: 목(골격독해) + 인성(의미연결) + 관성(경계인식) + 格순도
    //    배제: el.water, spike, noise 직접보너스, tg.식상(실행편향)
    const sub1 = subSkillStyled(
      1, "구조 파악력",
      [
        { name: "골격형",  raw: 1.7*el.wood + 1.2*tg.인성 + 1.2*tg.관성 + 0.7*geokPurity
                               - 0.30*tg.비겁 - 0.25*noise },
        { name: "판독형",  raw: 1.5*tg.관성 + 1.1*el.wood + 0.6*tg.인성 + 0.5*geokPurity
                               - 0.20*noise },
        { name: "연결형",  raw: 1.4*tg.인성 + 0.9*el.wood + 0.6*el.metal + 0.5*geokPurity
                               - 0.15*noise }
      ],
      0.55,
      [
        // 삼각 결합 보너스 — alone은 허용 안 함
        { cond: insightTriangle >= 0.14,                           add:  8, name: "인성+관성+목 삼각" },
        { cond: tg.인성 >= 0.22 && el.wood >= 0.20,               add:  5, name: "인성+목 결합" },
        { cond: tg.관성 >= 0.18 && el.wood >= 0.18,               add:  4, name: "관성+목 결합" },
        { cond: geokPurity >= 0.75,                                add:  4, name: "格 순도 높음" },
        // 패널티
        { cond: geokBroken === 1,                                  add: -6, name: "格 파격" },
        { cond: tg.비겁 >= 0.30 && tg.관성 < 0.12,               add: -6, name: "비겁 과다+관성 약함" },
        { cond: noise >= 2.5 && tg.인성 < 0.14,                  add: -6, name: "소음+인성부족" }
      ]
    );

    // 2) 패턴 해석력
    //    주재료: 인성(의미연결) + 목(패턴파악) + 인성+목 결합 시 insightTriangle 증폭
    //    배제: el.water, spike, tg.재성(실행편향), tg.식상 과다
    const sub2 = subSkillStyled(
      2, "패턴 해석력",
      [
        { name: "의미형",  raw: 1.8*tg.인성 + 1.1*el.wood + 0.5*tg.관성
                               + 3.0*insightTriangle          // 삼각 균형 폭발
                               - 0.30*noise - 0.35*tg.재성 },
        { name: "흐름형",  raw: 1.4*el.wood + 1.0*tg.인성 + 0.5*tg.관성 + 0.4*geokPurity
                               - 0.20*noise }
      ],
      0.55,
      [
        { cond: tg.인성 >= 0.22 && el.wood >= 0.20,                add:  7, name: "인성+목 패턴 결합" },
        { cond: insightTriangle >= 0.13,                           add:  5, name: "삼각 균형" },
        // 실행/기회 편향 패널티 — 식상 적정치(0.14~0.26)는 출력 보조로 허용
        // 과다(>=0.28)이면서 인성이 약할 때만 분산 패널티
        { cond: tg.식상 >= 0.28 && tg.인성 < 0.15,               add: -5, name: "식상 과다+인성 부족" },
        { cond: tg.재성 >= 0.28 && tg.인성 < 0.15,               add: -4, name: "재성 과다+인성 부족" },
        { cond: noise >= 2.5,                                      add: -5, name: "소음 과다" }
      ]
    );

    // 3) 리스크 감지력
    //    주재료: 관성(경계/질서) + 금(판단선) + 수는 보조(0.4), 충형파해 보조(0.25)
    //    배제: spike 직접가산, noise 직접보너스, 재성/식상 과다(낙관편향)
    const sub3 = subSkillStyled(
      3, "리스크 감지력",
      [
        { name: "경계형",  raw: 1.8*tg.관성 + 1.1*el.metal + 0.4*el.water
                               + 0.3*(chung+pa+hae)*0.22        // 합충형파 보조 — 소량만
                               - 0.20*noise },
        { name: "구조형",  raw: 1.5*tg.관성 + 0.9*el.metal + 0.6*el.wood + 0.4*geokPurity
                               - 0.15*noise }
      ],
      0.52,
      [
        { cond: tg.관성 >= 0.20 && el.metal >= 0.18,              add:  7, name: "관성+금 경계 결합" },
        { cond: tg.관성 >= 0.17 && (chung+hyung+pa+hae) >= 2,    add:  4, name: "관성+합충형파 민감" },
        // 낙관/즉흥 패널티
        { cond: tg.재성 >= 0.27 && tg.관성 < 0.13,               add: -7, name: "재성 과다+관성 부족" },
        { cond: tg.식상 >= 0.27 && tg.관성 < 0.13,               add: -5, name: "식상 과다+관성 부족" },
        { cond: noise >= 2.5 && tg.관성 < 0.14,                  add: -5, name: "소음+관성약함" }
      ]
    );

    // 4) 판단 정밀도
    //    주재료: 금(절단·결론) + 인성(정리) + 식상 일부(출력)
    //    패널티: 인성 과다(생각만 많음), 수 과다(흔들림), noise 과다
    const sub4 = subSkillStyled(
      4, "판단 정밀도",
      [
        { name: "절단형",  raw: 1.7*el.metal + 1.1*tg.인성 + 0.5*tg.식상 + 0.3*tg.관성
                               - 0.30*noise
                               - 0.40*Math.max(0, tg.인성 - 0.29) },  // 인성 과다 패널티
        { name: "정리형",  raw: 1.4*tg.인성 + 1.1*el.metal + 0.6*tg.관성 + 0.5*tg.식상  // 식상: 정리 출력 보조
                               - 0.30*noise
                               - 0.40*Math.max(0, tg.인성 - 0.29) }
      ],
      0.52,
      [
        { cond: el.metal >= 0.20 && tg.인성 >= 0.18,              add:  6, name: "금+인성 정밀 결합" },
        { cond: el.metal >= 0.17 && tg.식상 >= 0.14,              add:  3, name: "금+식상 출력 보조" },
        // 판단 마비 패널티
        { cond: tg.인성 >= 0.32,                                   add: -5, name: "인성 과다(분석마비)" },
        { cond: el.water >= 0.28 && el.metal < 0.14,              add: -5, name: "수 과다+금 부족" },
        { cond: noise >= 2.5,                                      add: -5, name: "소음 과다" }
      ]
    );

    // 5) 전략 설계력
    //    주재료: 인성(해석→계획) + 관성(리스크고려) + 인성×관성 결합 폭발
    //           + 토(안정실행) + 식상 일부(실행) + 비겁 적정치는 주체성 보조
    //    패널티: 비겁 과다(독선), 식상 과다(즉흥)
    const sub5 = subSkillStyled(
      5, "전략 설계력",
      [
        { name: "설계형",   raw: 1.5*tg.인성 + 1.4*tg.관성 + 0.6*el.earth + 0.5*tg.식상
                                + 2.5*Math.min(tg.인성, tg.관성)   // 인성+관성 결합 폭발
                                + 0.4*Math.min(tg.비겁, 0.22)      // 비겁 적정치: 주체성 보조 (최대 0.22까지)
                                + 0.3*connect - 0.20*noise },
        { name: "운영형",   raw: 1.3*tg.관성 + 1.1*tg.인성 + 0.7*el.earth + 0.4*connect
                                + 2.0*Math.min(tg.인성, tg.관성)
                                + 0.3*Math.min(tg.비겁, 0.22)      // 비겁 적정치 보조
                                - 0.20*noise }
      ],
      0.55,
      [
        { cond: tg.인성 >= 0.20 && tg.관성 >= 0.17,               add:  8, name: "인성+관성 설계 결합" },
        { cond: tg.인성 >= 0.18 && tg.관성 >= 0.14 && tg.식상 >= 0.14, add: 5, name: "설계+실행 삼각" },
        { cond: geokPurity >= 0.70,                                add:  3, name: "格 순도 뒷받침" },
        // 과다 패널티 (적정치 초과 시만)
        { cond: tg.비겁 >= 0.32 && tg.인성 < 0.15,               add: -5, name: "비겁 과다+인성 부족(독선)" },
    ...
    // ===== 대분류 평균 (강점 보존) =====
    const InsightRaw = strongAvg([sub1.score, sub2.score, sub3.score, sub4.score, sub5.score]);

    // ── 통찰력 80 임계 규칙 (GPT 원안)
    // 80 이상은 구조파악력(sub1) + 리스크감지력(sub3) + 전략설계력(sub5)
    // 이 3축이 동시에 ≥ 70일 때만 허용
    // → 감응형/예지형 명식은 통찰 80대 진입 불가
    const insightCoreTriple = sub1.score >= 70 && sub3.score >= 70 && sub5.score >= 70;
    let InsightAvg = InsightRaw;
    if (InsightAvg >= 80 && !insightCoreTriple) {
      InsightAvg = Math.min(InsightAvg, 79); // 3축 조건 미충족 시 79로 제한
    }
