...
console.log("🔥 intuition_engine.js WorkInsight v1.0 로드");
...
(function () {
  ...
  const DIST = {
    org:     { mean: 1.540, std: 0.573 },   // 조직 구조 파악력 (독해형 z≈2.5)
    risk:    { mean: 0.692, std: 0.267 },   // 업무 리스크 감지력 (독해형 z≈2.5)
    context: { mean: 1.915, std: 0.761 },   // 맥락 해석력 (실측 분포)
  };
  ...
  function compute(baseState) {
    ...
    let s_org = scoreFromZ(raw_org, DIST.org);
    if (elWood < 0.14) s_org = Math.min(s_org, 78);
    else if (elWood < 0.18) s_org = Math.min(s_org, 84);
    if (!(ci_관성 >= 0.26 && elWood >= 0.24 && geokPurity >= 0.72))
      s_org = Math.min(s_org, 92);

    // 2. 업무 리스크 감지력 (34%) — 관성 + 금 보조
    const raw_risk =
      1.85 * ci_관성
      + 0.55 * elMetal
      + 0.25 * stabilityIndex
    ...
    // 3. 맥락 해석력 (26%) — 인성 + rootScore + stemSupport
    const raw_context =
      2.05 * ci_인성
      + 1.05 * rootScore
      + 0.95 * stemSupport
    ...
    let s_context = scoreFromZ(raw_context, DIST.context);
    if (rootScore < 0.25) s_context = Math.min(s_context, 88);
    ...
    // 조직 구조 + 리스크 동시 강세 보너스
    const corePair  = (s_org + s_risk) / 2;
    const pairBonus = Math.max(0, corePair - 84) * 0.45;

    // 핵심 2축 중 하나라도 78 미만 → 감점
    const weakCount   = [s_org, s_risk].filter(v => v < 78).length;
    const weakPenalty = weakCount >= 1 ? 1.8 : 0;
    ...
    const sub1 = Math.round(s_org);
    const sub2 = Math.round(s_risk);
    const sub3 = Math.round(s_context);

    console.log(`📊 [WI v1.0] 조직${sub1} 리스크${sub2} 맥락${sub3}`);
    console.log(`🎯 [WI v1.0] 직장통찰 총점: ${insightTotal} | 90+조건: ${ninetyConditions}/3`);
    ...
    return {
      ...
      subs6: [
        { id:1, name:"조직 구조 파악력",   score:sub1, grade:grade(sub1), gradeLabel:gradeLabel(sub1) },
        { id:2, name:"업무 리스크 감지력", score:sub2, grade:grade(sub2), gradeLabel:gradeLabel(sub2) },
        { id:3, name:"맥락 해석력",        score:sub3, grade:grade(sub3), gradeLabel:gradeLabel(sub3) },
      ],
      ...
      debug: {
        ...
        raw: {
          org:     raw_org.toFixed(3),
          risk:    raw_risk.toFixed(3),
          context: raw_context.toFixed(3),
        },
        ninetyConditions,
      }
    };
  ...
  function _workInsightType(org, risk, ctx) {
    // 구조-리스크형: 두 핵심축 모두 매우 높음
    if (org >= 88 && risk >= 86)
      return { name:"구조-리스크형", desc:"조직 구조를 읽고 리스크를 먼저 잡는 직장형 통찰" };
    // 구조독해형: 조직 파악 최고, 리스크 중상
    if (org >= 86 && org > risk && org > ctx)
      return { name:"구조독해형", desc:"조직의 실제 권한 흐름과 병목을 먼저 읽는 통찰" };
    // 리스크판독형: 리스크 최고, 조직 중상
    if (risk >= 86 && risk > org && risk >= ctx)
      return { name:"리스크판독형", desc:"업무 실패 지점과 변수를 남보다 먼저 감지하는 통찰" };
    // 맥락해석형: 맥락 최고
    if (ctx >= 84 && ctx > org && ctx > risk)
      return { name:"맥락해석형", desc:"말의 표면보다 의도와 흐름을 읽는 해석형 통찰" };
    return { name:"균형통찰형", desc:"직장 통찰의 세 축이 균형 있게 발달한 유형" };
  }

  function _workInsightComment(org, risk, ctx) {
    const items = [
      { name:"조직 구조 파악력", score:org  },
      { name:"업무 리스크 감지력", score:risk },
      { name:"맥락 해석력",     score:ctx  },
    ];
  ...
  console.log("✅ IntuitionEngine WorkInsight v1.0 로드 완료");
})();
