...
/* ─────────────────────────────────────────
   한신 변동성 계산 (GPT 설계 — computeHanVolatility)
   · 허투 / 충형 / 파격 / 방향성 애매 / 구조계수 편차 조건 합산
   · 반환 범위: 0 ~ 1.0
───────────────────────────────────────── */
function computeHanVolatility(axis, state) {
  let v = 0;
  // 허투(虛透): 뿌리 없이 천간에 노출
  if ((axis._base?.rootNorm ?? 0) < 0.16 && (axis._base?.exposedNorm ?? 0) >= 0.42) v += 0.22;
  // critical 충
  if ((state.interactions?.충 || []).filter(c => c.critical).length >= 1) v += 0.16;
  // critical 형
  if ((state.interactions?.형 || []).filter(c => c.critical).length >= 1) v += 0.10;
  // 파격
  if (state.geok?.broken) v += 0.16;
  // 회복 — 조건부 작용성으로 반영
  if (state.geok?.recovery) v += 0.08;
  // 방향성 애매 (usefulnessIndex ≈ 0)
  if (Math.abs(axis.usefulnessIndex ?? 0) <= 0.18) v += 0.14;
  // 구조계수 편차
  if ((axis.structureCoeff ?? 1.0) <= 0.93 || (axis.structureCoeff ?? 1.0) >= 1.07) v += 0.10;
  return Math.min(1, v);
}

/* ─────────────────────────────────────────
   ROLE 판정 (GPT 설계 — resolveRole)
   · help-risk gap 직접 반영
   · 한신 = 조건부 변동성 축으로 재정의
   · 중립 구간 축소
───────────────────────────────────────── */
function resolveRole(axis, gods, grp, helpRiskGap = 0) {
  const ui         = axis.usefulnessIndex ?? 0;
  const sc         = axis.structureCoeff  ?? 1.0;
  const score      = axis.score           ?? 50;

  // axis에서 직접 _stateGeok/_stateInteractions 가져오기
  const volatility = computeHanVolatility(axis, {
    geok:         axis._stateGeok         || {},
    interactions: axis._stateInteractions || {}
  });

  // 상단
  if (score >= 66) return "용신";
  if (score >= 56) return "희신";

  // 중간 구간 세분화 (47~55)
  if (score >= 47 && score <= 55) {
    if (helpRiskGap >= 0.12 && ui >= 0.03) return "희신";
    if (helpRiskGap <= -0.12 && ui <= -0.03) return "기신";
    if (volatility >= 0.42) return "한신";
    return "중립";
  }

  // 하단 (≤44)
  if (score <= 44) {
    if (volatility >= 0.45 && Math.abs(ui) < 0.22) return "한신";
    return "기신";
  }
...
function computeResourceScores(state) {
  ...
  const axes = GROUPS.map(g => {
    ...
    // resolveRole: help-risk gap 기반 ROLE 보정 (GPT 설계 통합)
    const hrGap = (help - risk);  // help-risk gap (-1 ~ +1)

    // axis 임시 객체 구성 (resolveRole용)
    const axisTemp = {
      score,
      usefulnessIndex: ui,
      ...
      _base:           baseProfile[g],
      _stateGeok:      geok,
      _stateInteractions: interactions,
    };
    const resolvedRole  = resolveRole(axisTemp, gods, g, hrGap);
    const groupRoleFull = getGroupRole(g, gods, GROUP_MAP, ui, structureCoeff_[g]);

    // 최종 role: resolveRole이 한신/기신을 더 정확히 판정하므로 우선 적용
    // getGroupRole이 용신/희신으로 승격 판정한 경우는 그대로 존중
    let role;
    if (groupRoleFull === "용신" || groupRoleFull === "희신") {
      role = groupRoleFull;
    } else {
      // 한신/기신/중립 구간은 resolveRole로 재판정
      role = resolvedRole;
    }
