...
/* ─────────────────────────────────────────
   헬퍼 4: 그룹의 용희기한 역할 판정 (V4)
   ─────────────────────────────────────────
   5축 role은 두 십신의 단순 평균이 아니라
   "대표성(max) + 평균(avg)" 혼합으로 계산한다.
   즉 축 내부에 강한 용신/희신 십신이 하나라도 있으면
   축의 방향성이 중립에 묻히지 않게 한다.

   판정 우선순위:
   1) 그룹 내부 십신의 roleAnchor + rankedScore 조합
   2) usefulness / structure 보정
   3) 용신/희신 승격 규칙, 기신/한신 하향 규칙
   4) 최종 threshold 판정

   목표:
   - 중립 과다 완화
   - 축 내부 대표 십신의 방향성 반영
   - 용신이 UI에서 아예 사라지는 현상 방지
───────────────────────────────────────── */
function getGroupRole(grp, gods, GROUP_MAP, usefulnessIndex, structureCoeff) {
  ...
  // 4차 핵심:
  // 평균이 아니라 "대표성"을 주기 위해 max 쪽에 더 큰 가중
  ...
  // usefulness 영향력 소폭 강화
  const roleScore = groupBase + 11 * ui + 14 * (sc - 1.0);
  ...
  // 승격 규칙:
  // 축 내부에 용신/희신 십신이 있고 작용상 유리하면 중립에 묻히지 않게 승격
  ...
  // 하향 규칙:
  // 축 내부에 한신/기신 십신이 있고 작용상 불리하면 중립 방치 금지
  ...
  console.log(
    `🧭 [groupRole V4] ${grp} max=${maxScore.toFixed(1)} avg=${avgScore.toFixed(1)} base=${groupBase.toFixed(1)} ui=${ui.toFixed(2)} sc=${sc.toFixed(2)} y=${hasYong} h=${hasHee} g=${hasGi} hn=${hasHan} => ${roleScore.toFixed(1)}`
  );
