...
function computeResourceScores(state) {
  ...
  const axes = GROUPS.map(g => {
    ...
    // ── 4개 밴드 합산 → 20~220
    // powerBand  : 절대 작동량 → 0~150
    const powerBand    = 150 * Math.tanh(absPow / 6.2);
    // spreadBand : 상대 비중 보정 → -20~+20
    const spreadBand   = 20  * Math.tanh((shareRat - 0.20) / 0.09);
    // utilityBand: 유용성 지수 직접 반영 → -16~+16
    const utilityBand  = 16  * usefulnessIdx[g];
    // structureBand: 구조 보전도 직접 반영 → 대략 -4~+3
    const structureBand = 14 * (structureCoeff_[g] - 1.0);

    const score = _clamp(
      Math.round(20 + powerBand + spreadBand + utilityBand + structureBand),
      20, 220
    );
