...
function scoreDaeunPerformance(mergedState) {
  ...
  // B. 용신/희신이 식상/재성/관성 계열인지
  const PERF_FAMILY = ["식상","재성","관성"];
  let godsBonus = 0;
  const yongFam = gods.yong?.ranked?.map(r => {
    if (["食神","傷官"].includes(r.tg)) return "식상";
    if (["偏財","正財"].includes(r.tg)) return "재성";
    if (["偏官","正官"].includes(r.tg)) return "관성";
    return null;
  }).filter(Boolean) || [];
  const heeFam = gods.hee?.ranked?.map(r => {
    if (["食神","傷官"].includes(r.tg)) return "식상";
    if (["偏財","正財"].includes(r.tg)) return "재성";
    if (["偏官","正官"].includes(r.tg)) return "관성";
    return null;
  ...
  if (yongFam.some(f => PERF_FAMILY.includes(f))) godsBonus += 18;
  if (heeFam.some(f => PERF_FAMILY.includes(f)))  godsBonus += 8;
  ...
  [...giTGs, ...hanTGs].forEach(tg => {
    if (PERF_TG.includes(tg)) {
      const v = vectors.tenGods[tg] || 0;
      godsNeg += Math.min(v * 2.5, 8);
    }
  });
  godsBonus = Math.max(0, godsBonus - godsNeg);

  // C. 격의 실전성 (purity 기반, broken이어도 recover면 완전 박살 금지)
  const geokBase = geok.purity * 20;
  const geokAdj  = geok.broken && !geok.recovery ? -8 : geok.recovery ? 4 : 0;
  const geokScore = Math.max(0, geokBase + geokAdj);

  // D. strength 적정성 (너무 신약이면 성과 실행력 저하)
  const ss = strength.score;
  const strengthPenalty = ss < 30 ? (30 - ss) * 0.3 : 0;

  const raw = perfA + godsBonus + geokScore - strengthPenalty;
  return Math.round(_clamp(raw, 0, 100));
}

/* ─────────────────────────────────────────
   [B] 마찰 점수 (frictionScore)
   "소모·충돌·불안정" — 높을수록 마찰이 큼 (0~100)
   단, 생산적 재편(productive restructuring)은 약하게 반영
───────────────────────────────────────── */
function scoreDaeunFriction(mergedState) {
  const { vectors, gods, geok, strength, interactions } = mergedState;
  const totalTG = Object.values(vectors.tenGods).reduce((a,b) => a+b, 0) || 1;

  // A. 충/형/파/해 — destructive vs productive 구분
  // destructive: 기신/한신 작동 or 용신뿌리 직접 타격
  // productive: 용신/희신 활성 + geok.recovery or purity 상승 동반
  ...
  interactions.충.forEach(c => {
    ...
    // productive 신호: recover가 있거나 용신/희신이 강하게 살아 있으면 절반만 반영
    const isProductive = geok.recovery ||
      (yongTGs.length > 0 && (vectors.tenGods[yongTGs[0]] || 0) > 1.2);
    frictionRaw += isProductive ? base * 0.45 : base;
  });

  // 형 평가
  interactions.형.forEach(c => {
    frictionRaw += c.critical ? 6 : 3.5;
  });

  // 파/해 (항상 destructive 비중 낮음)
  interactions.파.forEach(c => { frictionRaw += c.critical ? 3 : 1.8; });
  interactions.해.forEach(c => { frictionRaw += c.critical ? 2 : 1.2; });

  // 합은 friction 완화 (안정·흐름 회복)
  frictionRaw -= interactions.합.length * 2.5;
  ...
  // C. 극단적 신강/신약 → friction 상승
  const ss = strength.score;
  const extremePenalty = ss <= 25 ? (25 - ss) * 0.5
                       : ss >= 78 ? (ss - 78) * 0.4
                       : 0;

  // D. 격 파괴 (broken + recover 없음)
  const geokFriction = geok.broken && !geok.recovery ? 10 : 0;

  const raw = frictionRaw + giHanFriction + extremePenalty + geokFriction;
  return Math.round(_clamp(raw, 0, 100));
}
...
/* ─────────────────────────────────────────
   ...
   "버티는 힘·회복력·안정감" (0~100)
───────────────────────────────────────── */
function scoreDaeunSupport(mergedState, baseStrengthScore) {
  const D       = window.SajuData;
  const { vectors, gods, geok, strength, interactions } = mergedState;
  const totalTG = Object.values(vectors.tenGods).reduce((a,b) => a+b, 0) || 1;

  // A. strength 재계산 결과
  // 중화(36~65) 구간이 가장 좋고, 극단으로 갈수록 감점
  const ss = strength.score;
  const strengthBase =
    ss >= 40 && ss <= 62 ? 30
  ...
  // 원국 대비 대운에서 strength가 개선됐으면 보너스
  const strengthDelta = ss - (baseStrengthScore || ss);
  const deltaBonus = _clamp(strengthDelta * 0.3, -6, 8);
  ...
  const raw = strengthBase + deltaBonus + supportA + stableBonus + recoverBonus + hapBonus;
  return Math.round(_clamp(raw, 0, 100));
}
...
/* ─────────────────────────────────────────
   [D-4] 대운 종합 점수 조합 (편차 합산 방식)
   ─────────────────────────────────────────
   공식: overall = 50
          + 0.45 * (performanceScore - 50)
          + 0.30 * (supportScore - 50)
          - 0.22 * (frictionScore - 50)

   의미:
   · 세 축 모두 50이면 overall = 50 (neutral 보장)
   · performance/support 상승 → overall 상승
   · friction 상승 → overall 하락
   · friction이 중간(50)이어도 성과·기반이 좋으면 65~80 도달
   · 특정 사용자·나이·간지 예외 없음
───────────────────────────────────────── */
function computeDaeunOverall(performanceScore, frictionScore, supportScore) {
  const raw = 50
    + 0.45 * (performanceScore - 50)
    + 0.30 * (supportScore     - 50)
    - 0.22 * (frictionScore    - 50);
...
───────────────────────────────────────── */
function computeDaeunScore(mergedState, baseStrengthScore) {
  // ① raw 계산 (기존 함수 = raw 함수로 사용)
  const rawPerformance = scoreDaeunPerformance(mergedState);
  const rawFriction    = scoreDaeunFriction(mergedState);
  const rawSupport     = scoreDaeunSupport(mergedState, baseStrengthScore);
  ...
  // ③ 종합 점수 (편차 합산)
  const overall = computeDaeunOverall(performance, friction, support);

  // ④ debug 로그 (raw vs normalized 구분 가능)
  console.debug("  🔢 [대운 raw→norm]",
    `rawPerf=${rawPerformance} → perf=${performance}`,
    `rawSupp=${rawSupport} → supp=${support}`,
    `rawFric=${rawFriction} → fric=${friction}`,
    `overall=${overall}`
  );

  return { overall, performance, friction, support,
...
window.SajuEngine = {
  ...
  // raw 계산 (기존 함수 — 해석 로직 변경 없음)
  scoreDaeunPerformance,
  scoreDaeunFriction,
  scoreDaeunSupport,
  // normalize 계층 (스케일 정상화 — 개인 예외 없음)
