...
/* ─────────────────────────────────────────
   헬퍼 4: 그룹의 용희기한 역할 판정 (3차)
   ─────────────────────────────────────────
   ROLE_ANCHOR(76/60/50/40/24)를 60% 우선, rankedScore를 40% 보조로 혼합해
   그룹 내 십신들의 groupBase를 산출한다.
   vectors.tenGods 재가중 없음 — 부족해도 희신/용신 역할은 살아 있어야 함.
   usefulness / structure는 최종 roleScore의 소폭 보정만 담당.
   dead zone(46~53): 애매한 축은 중립으로 보냄.
───────────────────────────────────────── */
function getGroupRole(grp, gods, GROUP_MAP, usefulnessIndex, structureCoeff) {
  ...
  // 역할 자체의 고전적 우선값 — "많다/적다"보다 "필요하냐/해가 되냐"를 먼저 반영
  const ROLE_ANCHOR = {
    "용신": 76,
    "희신": 60,
    "중립": 50,
  ...
  // 핵심: vectors.tenGods로 다시 가중하지 않는다.
  // 부족한 희신/용신도 role에서 살아 있어야 함.
  const tgScores = tgs.map(tg => {
    const role        = getTgRole(tg);
    const roleAnchor  = ROLE_ANCHOR[role] ?? 50;
    const rankedScore = rankedMap[tg] ?? 50;
    // roleAnchor 우선, rankedScore는 보조로만
    return 0.60 * roleAnchor + 0.40 * rankedScore;
  });

  const groupBase =
    tgScores.length > 0
      ? tgScores.reduce((a, b) => a + b, 0) / tgScores.length
      : 50;

  const ui = usefulnessIndex ?? 0;
  const sc = structureCoeff  ?? 1.0;

  // usefulness / structure는 role의 최종 보정값
  const roleScore = groupBase + 9 * ui + 12 * (sc - 1.0);

  console.log(
    `🧭 [groupRole] ${grp} base=${groupBase.toFixed(1)} ui=${ui.toFixed(2)} sc=${sc.toFixed(2)} => ${roleScore.toFixed(1)}`
  );

  // 3차: dead zone — 46~53 구간은 무조건 중립 (희신/기신 과다 판정 방지)
  if (roleScore > 46 && roleScore < 54) return "중립";

  if (roleScore >= 69) return "용신";
  if (roleScore >= 58) return "희신";
