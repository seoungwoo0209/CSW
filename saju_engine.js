...
function getGroupRole(grp, gods, GROUP_MAP, usefulnessIndex, structureCoeff) {
  ...
  // ── 하단 음성 규칙 재정의 (V6)
  // 제거: hasHan && ui < -0.08 && minScore <= 34 → 한신/기신 (minScore 단독 트리거)
  // 제거: hasGi && ui < -0.02 && minScore <= 42 → 기신 (조건 너무 느슨)
  // 변경: 한신은 ambiguous ui + 구조 불안정(sc) + hasHan 3조건 동시 필요
  //       기신은 명확한 음수 방향성(ui) + 기신 흔적 또는 강한 음수 ui

  // 한신: 방향성 애매 + 구조 흔들림 + hasHan 흔적 — 3조건 동시 필요
  const uiAmbiguous = Math.abs(ui) <= 0.16;
  const scUnstable  = sc < 0.96 || sc > 1.10;
  if (hasHan && uiAmbiguous && scUnstable) {
    return "한신";
  }

  // 기신: 명확한 음수 방향성 + 기신 흔적
  if (hasGi && ui <= -0.06 && roleScore <= 45) {
    return "기신";
  }
  // 기신: 방향성이 강하게 음수면 흔적 없어도 기신 가능
  if (ui <= -0.12 && roleScore <= 42) {
    return "기신";
  }

  console.log(
    `🧭 [groupRole V6] ${grp} max=${maxScore.toFixed(1)} avg=${avgScore.toFixed(1)} base=${groupBase.toFixed(1)} ui=${ui.toFixed(2)} sc=${sc.toFixed(2)} bias=${roleBias.toFixed(2)} y=${hasYong} h=${hasHee} g=${hasGi} hn=${hasHan} => ${roleScore.toFixed(1)}`
  );

  // ── 최종 일반 판정선 (V6)
  // 제거: roleScore <= 31 → 한신 자동 확정 (낮은 점수 하위 라벨 문제)
  // 변경: roleScore <= 42 기신 → 음수 방향성 조건 추가 필수
  if (roleScore >= 64) return "용신";
  if (roleScore >= 54) return "희신";
  // 하단: 점수만으로 한신/기신 확정 금지 — 방향성 조건 필수
  if (roleScore <= 42 && ui <= -0.06) return "기신";
  return "중립";
}
