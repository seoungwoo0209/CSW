    ...
    for (const [a, b] of D.EARTHLY_CLASHES) {
      if ((a === basePillars.month.branch && b === extraPillar.branch) ||
        ...
        if (restructureSignals >= 3) {
          // 재편형 충: purity 소폭 감점, broken은 유지 (월지 충은 항상 broken)
...
function scoreDaeunPerformance(mergedState, baseState) {
  ...
  PERF_TG.forEach(tg => { perfAmount += (vectors.tenGods[tg] || 0); });
  const perfRatio = perfAmount / totalTG;
  const perfA = perfRatio >= 0.25 && perfRatio <= 0.62
    ? 30
    : perfRatio < 0.25
      ? 30 * (perfRatio / 0.25)
      : 30 * (1 - (perfRatio - 0.62) / 0.38); // 0.62 이상부터 감점 시작
  ...
  // 계수 0.45→0.37, 상한 24→19, 하한 -10→-8
  const viab = evaluatePerformanceViability(mergedState, baseState);
  const viabilityBonus = _clamp((viab.viabilityScore - 50) * 0.37, -8, 19);
  ...
  if (baseState) {
    ...
    // 혼합형: 비율(A=24) + 절대량(B=2), 상한 12
    perfDeltaBonus = _clamp(perfDelta * 24 + perfAbsDelta * 2, -6, 12);

    purityDelta      = geok.purity - (baseState.geok?.purity ?? geok.purity);
    purityDeltaBonus = _clamp(purityDelta * 20, -5, 8);
  ...
  if (ss < 28) {
    strengthPenalty = viab.viabilityScore >= 65 ? 3 : 6;
  } else if (ss < 36) {
    strengthPenalty = viab.viabilityScore >= 60 ? 1 : 3;
  }
...
function scoreDaeunFriction(mergedState, baseState) {
  ...
  if (baseState) {
    ...
    if (viab.viabilityScore >= 68) productiveCount++;
    // 조건 8 (신규): supportCarrier 충분 — 안정축이 성과를 받쳐줌
    if (suppRatio >= 0.24) productiveCount++;
  } else {
  ...
  interactions.충.forEach(c => {
    ...
    if (isProductive) {
      frictionRaw += base * 0.34;          // productive: 완화 (구버전보다 관대하지 않게)
    } else if (isFullDestructive) {
      frictionRaw += base;                 // full destructive만 최대
    } else if (yongHeeHit) {
      frictionRaw += base * 0.82;          // 타격은 있지만 4중 미충족 → 중간
    } else {
      frictionRaw += base * 0.68;          // 타격 없음 → 낮게
    }
  });
  ...
  if (geok.broken && !geok.recovery) {
    if (viab.viabilityScore >= 65 && suppRatio >= 0.22) geokFriction = 6;      // 재편형 버팀
    else if (viab.viabilityScore >= 55)                  geokFriction = 8.5;    // 중간 손상
...
function computeDaeunOverall(performanceScore, frictionScore, supportScore, profileName = "overall") {
  let p_w = 0.49, s_w = 0.23, f_w = 0.20;
