...
function buildExtendedStateWithExtraPillar(basePillars, extraPillar) {
  ...
  if (monthHiddenPrimary) {
    ...
    // 대운 지지와 월지 충 여부 → 동적 purity 감점 / broken 판정
    // "월지 충 = 무조건 파격"이 아니라, 재편형/파괴형을 구분
    for (const [a, b] of D.EARTHLY_CLASHES) {
      if ((a === basePillars.month.branch && b === extraPillar.branch) ||
          (b === basePillars.month.branch && a === extraPillar.branch)) {

        // 재편 가능성 지표 사전 계산 (viability 헬퍼 없이 간단 추정)
        // 1) 대운 천간이 성과축/용신 방향 투출인가
        const daeunPerfSS = getShishen(dayStem, extraPillar.stem);
        const PERF_TG_SET = new Set(["食神","傷官","偏財","正財","偏官","正官"]);
        const daeunIsPerfAxis = daeunPerfSS && PERF_TG_SET.has(daeunPerfSS);

        // 2) 성과축이 다른 지지에도 통근하는가 (월지 외 지지 체크)
        let otherPerfRoot = 0;
        [basePillars.year.branch, basePillars.day.branch,
         basePillars.hour.branch, extraPillar.branch].forEach(br => {
        ...
        // 3) support carrier 간단 추정
        const SUPP_SET = new Set(["正印","偏印","比肩","正官"]);
        const totalTGv = Object.values(vectors.tenGods).reduce((a,b) => a+b, 0) || 1;
        let suppAmount = 0;
        Object.keys(vectors.tenGods).forEach(tg => {
          if (SUPP_SET.has(tg)) suppAmount += (vectors.tenGods[tg] || 0);
        });
        const suppRatioEst = suppAmount / totalTGv;

        // 재편형 판정: 3개 중 2개 이상 → 가벼운 패널티
        let restructureSignals = 0;
        if (daeunIsPerfAxis)    restructureSignals++;
        ...
        if (restructureSignals >= 3) {
          // 재편형 충: purity 소폭 감점, broken 미설정
          geok.purity = Math.max(0.1, geok.purity - 0.05);
          // geok.broken 유지 않음 — 재편형으로 처리
        } else if (restructureSignals >= 2) {
          // 부분 재편: purity 중간 감점, broken 설정
          geok.purity = Math.max(0.1, geok.purity - 0.10);
          geok.broken = true;
        } else {
          // 파괴형 충: 기존 수준
...
/* ─────────────────────────────────────────
   ...
   "결과가 나는가"
   절대량 + 변화량(비율+절대 혼합) + 실행가능성(viability) + 구조상태
   ⚠️ perfA 최적 구간 확장 (0.55 → 0.68)
   ⚠️ perfDeltaBonus: 비율delta + 절대량delta 혼합
   ⚠️ viabilityBonus 강화 (0.35 → 0.45)
   ⚠️ yongHeeBonusLite 보조신호로만 유지
...
function scoreDaeunPerformance(mergedState, baseState) {
  ...
  // ── [A] 성과축 절대량
  // 최적 구간 상한 0.55 → 0.68로 완화 (고비율 성과축 조기 감점 방지)
  // 단, viability가 낮은데 성과축만 많은 경우는 viabilityBonus에서 패널티
  const PERF_TG = ["食神","傷官","偏財","正財","偏官","正官"];
  let perfAmount = 0;
  PERF_TG.forEach(tg => { perfAmount += (vectors.tenGods[tg] || 0); });
  const perfRatio = perfAmount / totalTG;
  const perfA = perfRatio >= 0.25 && perfRatio <= 0.68
    ? 30
    : perfRatio < 0.25
      ? 30 * (perfRatio / 0.25)
      : 30 * (1 - (perfRatio - 0.68) / 0.32); // 0.68 이상부터 감점 시작

  // ── [B] 용/희 성과축 보조 신호 (보조만, 결정권은 perfA+viability+delta)
  ...
  if (yongFam.some(f => PERF_FAMILY.includes(f))) yongHeeBonusLite += 10;
  if (heeFam.some(f => PERF_FAMILY.includes(f)))  yongHeeBonusLite += 4;
  ...
  // ── [D] 실행 가능성 (viability) — 결정권 핵심 계층
  // 강화: 계수 0.35 → 0.45, 상한 20 → 24
  const viab = evaluatePerformanceViability(mergedState, baseState);
  const viabilityBonus = _clamp((viab.viabilityScore - 50) * 0.45, -10, 24);

  // ── [E] 변화량 (delta) — 비율 + 절대량 혼합형
  let perfDelta         = 0;
  let perfAbsDelta      = 0;
  ...
  if (baseState) {
    ...
    perfDelta    = (perfNow / totalTG) - (perfBase / baseTotalTG); // 비율 delta
    perfAbsDelta = perfNow - perfBase;                              // 절대량 delta
    // 혼합형: 비율 증가(A=30) + 절대량 증가(B=4)
    perfDeltaBonus = _clamp(perfDelta * 30 + perfAbsDelta * 4, -6, 16);
  ...
  // ── [G] destructivePenalty — 삼중 조건: 직접타격 + broken + recovery없음 + purity하락
  ...
  mergedState._perfDebug = {
    perfDelta, perfAbsDelta, purityDelta,
...
/* ─────────────────────────────────────────
   ...
   ⚠️ productive 판정 조건 확장 (viability/supportCarrier 포함)
   ⚠️ broken 패널티 동적화 (viability 기반 3단계)
   ⚠️ full destructive: 4중 조건일 때만
   ⚠️ 특정 사용자/나이/간지 예외 없음
───────────────────────────────────────── */
function scoreDaeunFriction(mergedState, baseState) {
  ...
  const totalTG = Object.values(vectors.tenGods).reduce((a,b) => a+b, 0) || 1;
  const dayStem = mergedState.pillars.day?.stem;
  ...
  // ── viability 사전 계산 (productive 판정에 사용)
  const viab = evaluatePerformanceViability(mergedState, baseState);
  const suppRatio = viab.supportCarrier;

  // ── productive restructuring 판정 (2개 이상이면 productive)
  // 확장: viability/supportCarrier 조건 추가
  ...
  if (baseState) {
    ...
    const PERF_TG_SET2 = new Set(["食神","傷官","偏財","正財","偏官","正官"]);
    const nowPerfInYH   = [...yongTGs, ...heeTGs].some(t => PERF_TG_SET2.has(t));
    const basePerfInYH  = [
      ...(baseState.gods?.yong?.tenGods || []),
      ...(baseState.gods?.hee?.tenGods  || [])
    ].some(t => PERF_TG_SET2.has(t));
    if (nowPerfInYH && !basePerfInYH) productiveCount++;
    // 조건 6: geok.recovery 있거나 broken 해소
    if (geok.recovery || (!geok.broken && baseState.geok?.broken)) productiveCount++;
    // 조건 7 (신규): viabilityScore 높음 — 실행 가능성 충분
    if (viab.viabilityScore >= 65) productiveCount++;
    // 조건 8 (신규): supportCarrier 충분 — 안정축이 성과를 받쳐줌
    if (suppRatio >= 0.22) productiveCount++;
  } else {
  ...
  // ── 충 평가 (full destructive: 4중 조건 엄격화)
  let frictionRaw = 0;
  const purityDropFric = baseState
    ? Math.max(0, (baseState.geok?.purity ?? geok.purity) - geok.purity)
    : 0;
  ...
  interactions.충.forEach(c => {
    ...
    const yongHeeHit = [...yongTGs, ...heeTGs].some(tg => {
      ...
      return branches.some(br => {
        const hs = D.HIDDEN_STEMS_RATIO[br] || [];
        return hs.some(({ stem }) => getShishen(dayStem, stem) === tg);
      });
    });

    // full destructive: 4중 조건 모두 충족 시만 full
    // 1) 용희 뿌리 직접 타격
    // 2) recovery 없음
    // 3) purity 하락 명확
    // 4) viability/supportCarrier 낮음
    const isFullDestructive = yongHeeHit
      && !geok.recovery
      && purityDropFric > 0.05
      && viab.viabilityScore < 55
      && suppRatio < 0.18;

    if (isProductive) {
      frictionRaw += base * 0.22;          // productive: 최대 완화
    } else if (isFullDestructive) {
      frictionRaw += base;                 // full destructive만 최대
    } else if (yongHeeHit) {
      frictionRaw += base * 0.75;          // 타격은 있지만 4중 미충족 → 중간
    } else {
      frictionRaw += base * 0.60;          // 타격 없음 → 낮게
    }
  });
  ...
  interactions.형.forEach(c => {
    ...
    frictionRaw += isProductive ? base * 0.50 : base * 0.85;
  });

  // 파/해 (강도 낮음)
  interactions.파.forEach(c => {
    frictionRaw += isProductive ? (c.critical ? 1.0 : 0.6) : (c.critical ? 2.5 : 1.5);
  });
  interactions.해.forEach(c => {
    frictionRaw += isProductive ? (c.critical ? 0.7 : 0.4) : (c.critical ? 1.8 : 1.0);
  });
  ...
  // D. 格 파괴 — 동적화 (viability/suppRatio 기반 3단계)
  let geokFriction = 0;
  if (geok.broken && !geok.recovery) {
    if (viab.viabilityScore >= 65 && suppRatio >= 0.22) geokFriction = 5;      // 재편형 버팀
    else if (viab.viabilityScore >= 55)                  geokFriction = 8;      // 중간 손상
    else                                                 geokFriction = 10;     // 파괴형
  }

  const raw = Math.max(0, frictionRaw) + giHanFriction + extremePenalty + geokFriction;

  mergedState._fricDebug = {
    productiveCount, isProductive, strengthDelta, purityDelta2, perfAxisDelta,
    viabilityScore: viab.viabilityScore, suppRatio: suppRatio.toFixed(3),
    geokFriction
  };
...
/* ─────────────────────────────────────────
   ...
   ─────────────────────────────────────────
   overall(기본):
     overall = 50 + 0.51*(p-50) + 0.24*(s-50) - 0.19*(f-50)
     → performance 비중 상향: 실전 성과 운 과소평가 방지
     → support 비중 하향: 조용한 안정 운 과대평가 방지
     → friction 비중 소폭 하향: 재편형 충 과대감점 방지

   money:  성과 최우선
     overall = 50 + 0.58*(p-50) + 0.18*(s-50) - 0.18*(f-50)

   love:   기반 중시 (기존 철학 유지하되 support 비중 재조정)
     overall = 50 + 0.42*(p-50) + 0.32*(s-50) - 0.22*(f-50)
...
function computeDaeunOverall(performanceScore, frictionScore, supportScore, profileName = "overall") {
  let p_w = 0.51, s_w = 0.24, f_w = 0.19;
  if (profileName === "money") { p_w = 0.58; s_w = 0.18; f_w = 0.18; }
  if (profileName === "love")  { p_w = 0.42; s_w = 0.32; f_w = 0.22; }
  const raw = 50
