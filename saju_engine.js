...
/* ─────────────────────────────────────────
   헬퍼 4: 그룹의 용희기한 역할 판정
───────────────────────────────────────── */
function getGroupRole(grp, gods, GROUP_MAP) {
  const tgs = GROUP_MAP[grp];
  const inGrp = (list) => (list || []).some(tg => tgs.includes(tg));
  if (inGrp(gods.yong?.tenGods)) return "용신";
  if (inGrp(gods.hee?.tenGods))  return "희신";
  if (inGrp(gods.gi?.tenGods))   return "기신";
  if (inGrp(gods.han?.tenGods))  return "한신";
  return "중립";
}
...
function computeResourceScores(state) {
  ...
  const axes = GROUPS.map(g => {
    ...
    // 용희기한 보정
    const role    = getGroupRole(g, gods, GROUP_MAP);
