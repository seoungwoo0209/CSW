...
/* ─────────────────────────────────────────
   [D] 대운 종합 점수 조합
   overall = 0.50*performance + 0.30*support - 0.20*friction
   → 성과·기반이 충분하면 friction이 있어도 과도하게 무너지지 않음
   → 편안하지만 성과 없으면 과대평가 안 됨
───────────────────────────────────────── */
function computeDaeunOverall(performanceScore, frictionScore, supportScore) {
  const raw = 0.50 * performanceScore
            + 0.30 * supportScore
            - 0.20 * frictionScore;
  return Math.round(_clamp(raw, 0, 100));
}
...
function computeDaeunScore(mergedState, baseStrengthScore) {
  const performance = scoreDaeunPerformance(mergedState);
  const friction    = scoreDaeunFriction(mergedState);
  const support     = scoreDaeunSupport(mergedState, baseStrengthScore);
