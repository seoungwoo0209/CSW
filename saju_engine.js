...
/* =========================================================
   ...
   ─────────────────────────────────────────────────────────
   3단계 구조:
     1) 내부 raw 계산 (표면+지장간+월지가중+통근)
     2) soft saturation → internal 0~1 정규화
     3) 화면 표시용 20~220 변환
   ========================================================= */
function computeResourceScores(state) {
  ...
  const dayStem = pillars.day.stem;
  const gods    = state.gods;  // yong/hee/gi/han 정보
  ...
  // ── 위치별 가중치
  const PILLAR_W  = { year:0.80, month:1.40, day:1.10, hour:0.90 };
  const ROLE_W    = { "정기":0.60, "중기":0.25, "여기":0.15 };
  const MONTH_BOOST = 1.35;  // 월지 지장간 추가 가중

  // ── STEP 1: raw 계산
  const raw = { 비겁:0, 식상:0, 재성:0, 관성:0, 인성:0 };
  ...
  stemSlots.forEach(({ stem, pos }) => {
    ...
    if (grp) raw[grp] += PILLAR_W[pos] || 0.85;
  });
  ...
  branchSlots.forEach(({ branch, pos, isMonth }) => {
    (D.HIDDEN_STEMS_BRANCH[branch] || []).forEach(({ stem, role }) => {
      const ss  = getShishen(dayStem, stem);
      const grp = GROUPS.find(g => GROUP_MAP[g].includes(ss));
      if (!grp) return;
      const roleW   = ROLE_W[role] ?? 0.15;
      const pillarW = PILLAR_W[pos] || 0.85;
      const boost   = isMonth ? MONTH_BOOST : 1.0;
      raw[grp] += roleW * pillarW * boost;
    });
  });

  // ── STEP 2 + 3: 절대 성분 + 상대 성분 합산 → 20~220 변환
  // 절대 성분: 전체 대비 비중(ratio)이 균등(0.20) 기준 → 110점 중심
  // 상대 성분: tanh 기반 ±25 차별화
  // 균등 분포 → 전 항목 110 / 한 축 독점 → 최대 190~210 수준
  const rawTotal = GROUPS.reduce((s, g) => s + raw[g], 0) || 1;
  const display = {};
  GROUPS.forEach(g => {
    const ratio   = raw[g] / rawTotal;
    const A       = 110 + (ratio - 0.20) * 200;   // 절대 성분 (ratio=0.20→110)
    const relDev  = (ratio - 0.20) * 3.5;
    const B       = Math.tanh(relDev) * 25;        // 상대 성분 ±25
    display[g]    = Math.round(Math.max(20, Math.min(220, A + B)));
  });

  // ── 용신/희신/기신/한신 역할 매핑
  // gods.yong/hee/gi/han 각각의 tenGods 목록으로부터 그룹 결정
  function getRole(grp) {
    const tgs = GROUP_MAP[grp];
    const inGroup = (list) => list.some(tg => tgs.includes(tg));
    if (inGroup(gods.yong?.tenGods || [])) return "용신";
    if (inGroup(gods.hee?.tenGods  || [])) return "희신";
    if (inGroup(gods.gi?.tenGods   || [])) return "기신";
    if (inGroup(gods.han?.tenGods  || [])) return "한신";
    return "중립";
  }
  ...
  // 한 줄 요약
  const summary = `${strongest.key}(${strongest.score}) 중심의 사주 · ${weakest.key}(${weakest.score}) 보완 필요`;

  console.log("🎯 5축 자원 점수:", axes.map(a => `${a.key}:${a.score}(${a.role})`).join(" "));

  return { axes, strongest, weakest, summary };
}
