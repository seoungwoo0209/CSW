...
function computeResourceScores(state) {
  ...
  const axes = GROUPS.map(g => {
    ...
    // ── 4개 밴드 합산 → 20~220 (6.5 보정: 양적강도↓ 유용성/구조↑)
    // powerBand  : 절대 작동량 → 0~120 (기존 150 → 120, 양적 강도 비중 축소)
    const powerBand    = 120 * Math.tanh(absPow / 6.2);
    // spreadBand : 상대 비중 보정 → -15~+15 (기존 ±20 → ±15)
    const spreadBand   = 15  * Math.tanh((shareRat - 0.20) / 0.09);
    // utilityBand: 유용성 지수 직접 반영 → -28~+28 (기존 ±16 → ±28, 쓸모 비중 확대)
    const utilityBand  = 28  * usefulnessIdx[g];
    // structureBand: 구조 보전도 직접 반영 → 계수 22 (기존 14 → 22, 구조 비중 확대)
    const structureBand = 22 * (structureCoeff_[g] - 1.0);
