...
/* ─────────────────────────────────────────
   헬퍼 4: 그룹의 용희기한 역할 판정 (HanFix Final)
   ─────────────────────────────────────────
   5축 role은 "대표성(max 65% + avg 35%)" 혼합으로 계산한다.
   vectors.tenGods 재가중 없음.

   한신 철학:
   - 기신 = 현재 명확히 해로운 축 (최하단 endpoint)
   - 한신 = 중립 하방의 조건부/가변 축 (45~53 구간, hasHan 조건)
   - 한신을 low-score endpoint에서 완전 제거
───────────────────────────────────────── */
function getGroupRole(grp, gods, GROUP_MAP, usefulnessIndex, structureCoeff) {
  ...
  if (grp === "인성") {
    ...
    if ((hasHee || hasYong) && ui >= 0.04 && groupBase >= 54) return "희신";
  }

  // 재성 하향 규칙
  if (grp === "재성") {
    if ((hasGi || hasHan) && ui <= -0.04 && roleScore <= 46) return "기신";
    if (!hasYong && !hasHee && ui < 0 && roleScore <= 45)    return "기신";
    if (hasHan && ui <= -0.10 && minScore <= 42)             return "기신";
  }
  ...
  console.log(
    `🧭 [groupRole HanFix Final] ${grp} base=${groupBase.toFixed(1)} ui=${ui.toFixed(2)} sc=${sc.toFixed(2)} hasHan=${hasHan} hasGi=${hasGi} score=${roleScore.toFixed(1)}`
  );

  if (roleScore >= 64) return "용신";
  if (roleScore >= 54) return "희신";

  // 한신: 중립 하방 조건부 — low-score endpoint 아님
...
function computeResourceScores(state) {
  ...
  /* ── absolutePower 계산 헬퍼 (private)
     axisProfile의 기존 계수를 그대로 곱하므로 중복 재계산 없음
     getAxisCoeff 미포함 / month·root·exposed 원천 재계산 없음 */
  function _computeAbsolutePower(g) {
    ...
    // axisProfile.absoluteBasePower = amountRaw × monthCmd × rootCoeff × exposedCoeff
    // (3단계에서 이미 계산된 값 — 중복 계산 없이 참조)
    const base = ap.absoluteBasePower ?? (
      ap.amountRaw * ap.monthCommandCoeff * ap.rootCoeff * ap.exposedCoeff
    );
  ...
  const axes = GROUPS.map(g => {
    ...
    // ── presenceFloorBand: 약한 축 vs 거의 없는 축 분리
    const presenceCore =
    ...
    const presenceFloorBand =
      nearAbsent ? 0 : 26 * Math.pow(presenceCore, 0.72);

    // ── powerBand: 절대 작동량 (0~120)
    const powerBand = 120 * Math.tanh(absPow / 6.5);

    // ── spreadBand: 상대 비중 보정 — 패널티 완화 (±8)
    const spreadBand = 8 * Math.tanh((shareRat - 0.20) / 0.11);

    // ── utilityBand: 양수/음수 비대칭 — 불리한 축도 존재 자체를 죽이지 않음
    const ui = usefulnessIdx[g];
    const utilityBand = ui >= 0 ? 22 * ui : 12 * ui;

    // ── structureBand: 구조 보전도 (계수 18 — 점수를 뒤집지 않는 수준)
    const structureBand = 18 * (structureCoeff_[g] - 1.0);
