...
function computeAxisBaseProfile(state) {
  ...
  GROUPS.forEach(grp => {
    ...
    const absoluteBasePower =
      6.0 * (
        0.62 * _clamp(amountRaw[grp] / 3.2, 0, 1) +
        0.23 * rootObj.norm +
        0.15 * expObj.norm
      ) * monthCmd;
...
function getGroupRole(grp, gods, vectors, GROUP_MAP, usefulnessIndex, structureCoeff) {
  ...
  const roleScore = groupBase + 10 * ui + 14 * (sc - 1.0);

  if (roleScore >= 67) return "용신";
  if (roleScore >= 57) return "희신";
  if (roleScore <= 33) return "한신";
  if (roleScore <= 43) return "기신";
  return "중립";
}
