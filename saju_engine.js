...
function computeResourceScores(state) {
  ...
  // ── STEP 3: 점수화 — 20~220 체계 복구 (6단계 선반영 롤백)
  //   내부 계산(effectiveRaw, usefulnessIdx, structureCoeff)은 유지
  //   최종 사용자 표시 score만 기존 20~220 범위로 복구
  ...
  const axes = GROUPS.map(g => {
    ...
    // usefulnessIndex와의 중복은 약화 수준으로 유지 (5단계)
    const roleAdj = { 용신:5, 희신:3, 중립:0, 기신:-4, 한신:-6 }[role] ?? 0;

    const { help, risk } = getGroupHelpRisk(g, helpRisk, GROUP_MAP);
    const effAdj = (help - risk) * 6;   // 5단계 약화 수준 유지
  ...
  console.log("🎯 [V5] 5축 자원 점수:",
    axes.map(a => `${a.key}:${a.score}(${a.role}) eff=${a.effectiveRaw} ui=${a.usefulnessIndex}`).join(" ")
  );
