...
/* ─────────────────────────────────────────
   ...
   "결과가 나는가" — 절대량(60%) + 변화량(40%) 혼합형
   ⚠️ baseState 추가: 원국 대비 대운 변화량 반영
   ⚠️ 특정 사용자/나이/간지 예외 없음 — 구조 delta 범용 계산
───────────────────────────────────────── */
function scoreDaeunPerformance(mergedState, baseState) {
  ...
  [...giTGs, ...hanTGs].forEach(tg => {
    if (PERF_TG.includes(tg)) godsNeg += Math.min((vectors.tenGods[tg] || 0) * 2.5, 8);
  });
  ...
  if (baseState) {
    ...
    perfDeltaBonus = _clamp(perfDelta * 40, -8, 18);

    // B. purity 변화량
    purityDelta      = geok.purity - (baseState.geok?.purity ?? geok.purity);
    purityDeltaBonus = _clamp(purityDelta * 28, -8, 14);

    // C. recovery / broken 전환 신호
    const wasRecovery = baseState.geok?.recovery ?? false;
    const wasBroken   = baseState.geok?.broken   ?? false;
    if (!wasRecovery && geok.recovery)           recoveryTransBonus += 7;
    if (wasBroken    && !geok.broken)             recoveryTransBonus += 5;

    // D. 용/희의 성과축 전환
    const baseYongFam = (baseState.gods?.yong?.ranked || []).map(r => _toFam(r.tg)).filter(Boolean);
    const baseHeeFam  = (baseState.gods?.hee?.ranked  || []).map(r => _toFam(r.tg)).filter(Boolean);
    const nowInYong   = yongFam.some(f => PERF_FAMILY.includes(f));
    const nowInHee    = heeFam.some(f => PERF_FAMILY.includes(f));
    const wasInYong   = baseYongFam.some(f => PERF_FAMILY.includes(f));
    const wasInHee    = baseHeeFam.some(f => PERF_FAMILY.includes(f));
    if (nowInYong && !wasInYong) yongHeeShiftBonus += 8;
    if (nowInHee  && !wasInHee)  yongHeeShiftBonus += 4;
  }

  const raw = perfA + godsBonus + geokScore - strengthPenalty
            + perfDeltaBonus + purityDeltaBonus + recoveryTransBonus + yongHeeShiftBonus;

  // debug용 delta 값 외부에서 접근 가능하도록 state에 임시 첨부
  mergedState._perfDebug = { perfDelta, purityDelta, perfDeltaBonus, purityDeltaBonus, recoveryTransBonus, yongHeeShiftBonus };
