...
function calculateStrength(pillars) {
  ...
  // 라벨 기준: 68↑신강 / 45~67중화 / 44↓신약
  const label = total >= 68 ? "신강" : total >= 45 ? "중화" : "신약";
...
function computeTenGodClassicalScores(state) {
  ...
  TEN_GODS.forEach(tg => {
    ...
    if (pref.prefer?.includes(tg))  { usefulnessRaw += 0.44; notes.push("격prefer"); }
    if (pref.support?.includes(tg)) { usefulnessRaw += 0.20; notes.push("격support"); }
    if (pref.avoid?.includes(tg))   { usefulnessRaw -= 0.36; notes.push("격avoid"); }
    if (geok?.main === `${tg}격`)   { usefulnessRaw += 0.24; notes.push("격핵심"); }
    if (geok?.broken && pref.prefer?.includes(tg)) { usefulnessRaw -= 0.12; notes.push("격파손"); }
    // 고전 보정
...
function getAxisUsefulnessIndex(grp, state, axisProfile, tenGodScores) {
  ...
  let geokFit  = 0;
  tgs.forEach(tg => {
    if (pref.prefer?.includes(tg))  { geokFit += 0.28; reasons.push(`格prefer(${tg})`); }
    if (pref.support?.includes(tg)) { geokFit += 0.15; reasons.push(`格support(${tg})`); }
    if (pref.avoid?.includes(tg))   { geokFit -= 0.24; reasons.push(`格avoid(${tg})`); }
    if (geok?.main === `${tg}격`)   { geokFit += 0.12; reasons.push(`格핵심(${tg})`); }
  });
  if (geok?.broken) { geokFit -= 0.06; reasons.push("格파손"); }
  ...
  // ── D. presencePenalty: 존재 미약 시 usefulness 약화
  let presencePenalty = 0;
  const amtRaw = axisProfile?.amountRaw ?? 0;
  if (amtRaw < 0.25) { presencePenalty = -0.28; reasons.push(`존재매우약(${amtRaw})`); }
  else if (amtRaw < 0.55) { presencePenalty = -0.15; reasons.push(`존재약(${amtRaw})`); }
  ...
  const index = _clamp(
    ...
    0.25 * geokFit     +
    0.15 * stabilityFit +
    0.10 * presencePenalty,
    -1.0, 1.0
  );
  const adj = _round3(index * 0.22);
...
function classifyYongHeeGiHan(state) {
  ...
  console.log("🎯 [용희기한 V3-5단계] ranked:", ranked.map(x=>`${x.tg}:${x.score}`).join(" "));
...
function computeAxisBaseProfile(state) {
  ...
  GROUPS.forEach(grp => {
    ...
    const absoluteBasePower =
      amountRaw[grp] * monthCmd * rootObj.coeff * expObj.coeff;

    profile[grp] = {
      amountRaw:         _round2(amountRaw[grp]),
      monthCommandCoeff: _round3(monthCmd),
      // rootRaw = 통근량 (BRANCH 기반 정본 — getAxisRootCoeff().raw)
      // 보정: amountRaw 내 RATIO 기반 hidden 누적과 중복되지 않게 BRANCH raw를 정본으로 사용
      rootRaw:           _round2(rootObj.raw),
      rootNorm:          _round3(rootObj.norm),
      rootCoeff:         _round3(rootObj.coeff),
      exposedRaw:        _round2(stemPower[grp]),   // 기존 stemPower 하위호환
...
function computeResourceScores(state) {
  ...
  // ── STEP 3: 6단계 — absolutePower 기반 단일 점수 공식
  //   기존 amountScore + shareScore 이중 성분을 단일 absolutePower로 통합
  //   점수 범위: 20~220 → 0~100으로 정규화 후 UI 스케일로 변환
  const totalEff = GROUPS.reduce((s,g) => s + effectiveRaw[g], 0) || 1;
  const maxEff   = Math.max(...GROUPS.map(g => effectiveRaw[g]), 0.01);

  // 절대 강도 밴드 앵커: effectiveRaw → absolutePower (0~100)
  // - maxEff 기준 정규화 + 비중(shareRatio) 보정 혼합
  // - 5분의1(20%) 이상 비중이면 안정 구간, 과집중(40% 이상)은 감점
  function computeAbsolutePower(g) {
    const absNorm   = effectiveRaw[g] / maxEff;          // 0~1 절대 강도
    const shareRat  = effectiveRaw[g] / totalEff;        // 0~1 상대 비중
    // 절대 강도 성분 (0~70)
    const absScore  = absNorm * 70;
    // 비중 성분: 20% 기준 tanh 변환 (-15~+15)
    const shareScore = Math.tanh((shareRat - 0.20) * 5.0) * 15;
    // structureCoeff 직접 반영 (-10~+10)
    const structBonus = (structureCoeff_[g] - 1.0) * 35;
    // usefulnessIndex 직접 반영 (-12~+12)
    const usefulBonus = usefulnessIdx[g] * 12;
    return _clamp(absScore + shareScore + structBonus + usefulBonus, 0, 97);
  }
  ...
  function getStatus(score) {
    ...
    if (score >= 66) return "강한 편";
    if (score >= 48) return "보통";
    if (score >= 32) return "약한 편";
    return "매우 약함";
  }

  const axes = GROUPS.map(g => {
    const shareRatio    = effectiveRaw[g] / totalEff;
    const absolutePower = computeAbsolutePower(g);

    const role    = getGroupRole(g, gods, vectors, GROUP_MAP);
    // 6단계: role 보정은 absolutePower 위에 미세 조정 역할만
    const roleAdj = { 용신:4, 희신:2, 중립:0, 기신:-3, 한신:-5 }[role] ?? 0;

    const { help, risk } = getGroupHelpRisk(g, helpRisk, GROUP_MAP);
    const effAdj = (help - risk) * 4;   // 6단계: 더욱 약화

    // 최종 score: 0~100 정규화 점수 (UI에서 백분율처럼 읽힘)
    const score = _clamp(Math.round(absolutePower + roleAdj + effAdj), 0, 100);
    ...
    return {
      ...
      // 6단계 신규
      absolutePower:   _round2(absolutePower),
      _base: {
        ...baseProfile[g],
  ...
  console.log("🎯 [V6] 5축 자원 점수:",
    axes.map(a => `${a.key}:${a.score}(${a.role}) eff=${a.effectiveRaw} abs=${a.absolutePower}`).join(" ")
  );
