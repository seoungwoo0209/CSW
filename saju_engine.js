...
function computeTenGodClassicalScores(state) {
  ...
  TEN_GODS.forEach(tg => {
    ...
    // ── 한신용 보조지표
    let centralityRaw = 0;
    centralityRaw += 0.45 * presence;
    if (monthRoot) centralityRaw += 0.30;
    if (dayRoot)   centralityRaw += 0.18;
    if (geok?.main === `${tg}격`) centralityRaw += 0.18;
    const pref2 = D.GEOK_PREFERENCE?.[geok?.main] || D.GEOK_PREFERENCE?.["혼합격"] || {};
    if (pref2.prefer?.includes(tg))  centralityRaw += 0.10;
    if (pref2.support?.includes(tg)) centralityRaw += 0.06;
    const centrality = _clamp(centralityRaw, 0, 1);

    const netUsefulness = usefulness; // 이미 -1~+1

    let conditionalityRaw = 0;
    if (presence >= 0.12 && presence <= 0.58) conditionalityRaw += 0.22;
    if (presence >= 0.18 && structureNorm >= 0.40 && structureNorm <= 0.72) conditionalityRaw += 0.14;
    const interactionCount =
      (interactions?.충?.length || 0) +
      (interactions?.형?.length || 0) +
      (interactions?.파?.length || 0) +
      (interactions?.해?.length || 0);
    if (interactionCount >= 2) conditionalityRaw += 0.16;
    conditionalityRaw += Math.max(0, 0.28 - Math.abs(netUsefulness)) * 0.90;
    if (geok?.broken)   conditionalityRaw += 0.08;
    if (geok?.recovery) conditionalityRaw += 0.06;
    const conditionality = _clamp(conditionalityRaw, 0, 1);

    result[tg] = {
      score,
      presence:       _round3(presence),
      structure:      _round3(structureNorm),
      usefulness:     _round3(usefulness),
      centrality:     _round3(centrality),
      netUsefulness:  _round3(netUsefulness),
      conditionality: _round3(conditionality),
      notes
    };
  });
...
function classifyYongHeeGiHan(state) {
  ...
  const positiveNames = [...yongNames, ...heeItems.map(x => x.tg)];

  // ── 한신: "낮은 점수"가 아니라 "조건부" — 억지 fallback 없음
  let hanItems = ranked.filter(x => {
    const c  = x.detail.centrality     ?? 0;
    const nu = x.detail.netUsefulness  ?? 0;
    const cd = x.detail.conditionality ?? 0;
    const sc = x.score ?? 50;
    const lowCentrality   = c  <= 0.42;
    const neutralUse      = Math.abs(nu) <= 0.16;
    const highConditional = cd >= 0.52;
    const notBottomCrash  = sc >= 32;
    const notPositive     = !positiveNames.includes(x.tg);
    return lowCentrality && neutralUse && highConditional && notBottomCrash && notPositive;
  }).slice(0, 2);

  const hanNames = hanItems.map(x => x.tg);

  // ── 기신: 명확히 음의 유불리 (한신과 exclusive)
  let giItems = [...ranked].reverse().filter(x => {
    const nu = x.detail.netUsefulness ?? 0;
    const sc = x.score ?? 50;
    return !positiveNames.includes(x.tg) && !hanNames.includes(x.tg) &&
           (nu <= -0.18 || sc <= 40);
  }).slice(0, 2);

  // fallback은 기신에만 허용
  if (!giItems.length) {
    giItems = [...ranked].reverse()
      .filter(x => !positiveNames.includes(x.tg) && !hanNames.includes(x.tg))
      .slice(0, 1);
  }
  ...
  function pack(items) {
    return {
      ...
      ranked:   items.map(x => ({
        ...
        family:         x.family,
        element:        x.element,
        centrality:     x.detail?.centrality,
        netUsefulness:  x.detail?.netUsefulness,
        conditionality: x.detail?.conditionality
      }))
    };
  }
  ...
  console.log(
    ...
    ranked.map(x => {
      const d = x.detail || {};
      return `${x.tg}:${x.score}(c=${d.centrality},u=${d.netUsefulness},cd=${d.conditionality})`;
    }).join(" ")
  );
...
function scoreYongHeeGiHan(gods, vectors) {
  ...
  gods.han.tenGods.forEach(tg => {
    const c = vectors.tenGods[tg] || 0;
    if (c > 2.2) score -= Math.min((c - 2.2) * 0.6, 1.5);
  });
...
function getGroupRole(grp, gods, GROUP_MAP, usefulnessIndex, structureCoeff) {
  ...
  const ROLE_ANCHOR = {
    ...
    "희신": 62,
    "중립": 50,
    "한신": 46,   // 조건부 라벨 — 중립 하방이지 최하위 악성 아님
    "기신": 34,
  };
  ...
  console.log(
    `🧭 [groupRole HanFix] ${grp} base=${groupBase.toFixed(1)} ui=${ui.toFixed(2)} sc=${sc.toFixed(2)} hasHan=${hasHan} hasGi=${hasGi} score=${roleScore.toFixed(1)}`
  );
  ...
  // 한신은 하단 endpoint 가 아니라 중립 하방의 조건부 분기
  if (hasHan && Math.abs(ui) <= 0.14 && roleScore >= 45 && roleScore <= 53) {
    return "한신";
  }

  // 명확히 불리하면 기신 — 기신이 진짜 하단 담당
  if (roleScore <= 42) return "기신";
...
function computeResourceScores(state) {
  ...
  const axes = GROUPS.map(g => {
    ...
    // getGroupRole에 usefulnessIndex + structureCoeff 직접 전달
    const role = getGroupRole(g, gods, GROUP_MAP, ui, structureCoeff_[g]);

    // help/risk: 소폭 보조 (5단계 수준 유지, 최종 score에는 미반영 — 디버그용)
  ...
  console.log("🎯 [V6] 5축 자원 점수:",
    axes.map(a => `${a.key}:${a.score}(${a.role}) abs=${a.absolutePower} ui=${a.usefulnessIndex}`).join(" ")
  );
...
/* ── 공통 상수 ───────────────────────────────────────────── */
const ROLE_VALUE  = { "용신": 2, "희신": 1, "중립": 0, "한신": -0.25, "기신": -1 };
...
function scoreFlowFriction(state) {
  ...
  let giHanBurden = 0;
  // 기신은 직접 마찰
  (state.gods.gi?.tenGods || []).forEach(tg => {
    const v = state.vectors.tenGods[tg] || 0;
    if (v > 0.8) giHanBurden += Math.min(v * 2.5, 8);
  });

  // 한신은 직접 마찰이 아니라 "조건 변동성"만 소폭 반영
  (state.gods.han?.tenGods || []).forEach(tg => {
    const v = state.vectors.tenGods[tg] || 0;
    if (v > 1.6) giHanBurden += Math.min((v - 1.6) * 0.8, 1.8);
  });
