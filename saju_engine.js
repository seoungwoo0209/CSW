...
───────────────────────────────────────── */
function getGroupRole(grp, gods, vectors, GROUP_MAP, usefulnessIndex, structureCoeff) {
  const tgs = GROUP_MAP[grp];
  ...
  // usefulness + structure를 직접 혼합 → 중립 구간 축소 (45~53만 남음)
  const ui = usefulnessIndex ?? 0;
  const sc = structureCoeff  ?? 1.0;
  const roleScore = groupScore + 12 * ui + 20 * (sc - 1.0);

  if (roleScore >= 64) return "용신";
  if (roleScore >= 54) return "희신";
  if (roleScore <= 34) return "한신";
  if (roleScore <= 44) return "기신";
  return "중립";
}
...
function computeResourceScores(state) {
  ...
  const axes = GROUPS.map(g => {
    ...
    const role = getGroupRole(g, gods, vectors, GROUP_MAP, ui, structureCoeff_[g]);
